import { Trie } from "./try-hard.js";

export const colors = [
  {
    "title": "Silent White - Pale™",
    "number": "328",
    "lightness": 84,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Travertine - Light™",
          "Silent White - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Apple™",
          "Carys™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Light Bronze Green™"
        ]
      }
    ],
    "color": "#FBFDF2",
    "hsl": {
      "h": 71,
      "s": 73.3,
      "l": 97.1
    },
    "categories": [
      "Neutral",
      "White"
    ]
  },
  {
    "title": "Silent White™",
    "number": "329",
    "lightness": 77,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Travertine - Mid™",
          "Silent White - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Light Gold™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#F5F3E2",
    "hsl": {
      "h": 54,
      "s": 48.7,
      "l": 92.4
    },
    "categories": [
      "Neutral",
      "White"
    ]
  },
  {
    "title": "Linen Wash™",
    "number": "33",
    "lightness": 79,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "Portland Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Roman Plaster™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Boxington®"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#F4F3E7",
    "hsl": {
      "h": 55,
      "s": 37.1,
      "l": 93.1
    },
    "categories": [
      "Neutral",
      "White",
      "Colours of England"
    ]
  },
  {
    "title": "Stock®",
    "number": "37",
    "lightness": 76,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone™",
          "Travertine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Oak Apple™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Woad™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Vulcan™"
        ]
      }
    ],
    "color": "#F8F4E3",
    "hsl": {
      "h": 49,
      "s": 60,
      "l": 93.1
    },
    "categories": [
      "Neutral",
      "White",
      "Colours of England",
      "Stone"
    ]
  },
  {
    "title": "White Lead™",
    "number": "74",
    "lightness": 82,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Green Stone™",
          "Lute™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Gold™",
          "Boxington®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Pea Green™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#F9F8E3",
    "hsl": {
      "h": 57,
      "s": 64.7,
      "l": 93.3
    },
    "categories": [
      "Neutral",
      "White",
      "Archive",
      "Colours of England",
      "Georgian"
    ],
    "aliases": [
      "Muslin™"
    ]
  },
  {
    "title": "First Light™",
    "number": "49",
    "lightness": 88,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Green Stone - Light™",
          "Green Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Oak Apple™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#FCF6D8",
    "hsl": {
      "h": 50,
      "s": 85.7,
      "l": 91.8
    },
    "categories": [
      "Neutral",
      "White",
      "Colours of England"
    ]
  },
  {
    "title": "Hollyhock™",
    "number": "25",
    "lightness": 76,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Light Beauvais™",
          "Rolling Fog - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Nether Red™",
          "Masquerade™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#F3EEE8",
    "hsl": {
      "h": 33,
      "s": 31.4,
      "l": 93.1
    },
    "categories": [
      "Neutral",
      "White",
      "Colours of England"
    ]
  },
  {
    "title": "Joanna®",
    "number": "130",
    "lightness": 68,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mushroom™",
          "Rolling Fog - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Roman Plaster™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Attic II™"
        ]
      }
    ],
    "color": "#EBEAE0",
    "hsl": {
      "h": 55,
      "s": 21.6,
      "l": 90
    },
    "categories": [
      "Neutral",
      "Colours of England",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Joanna®"
    ]
  },
  {
    "title": "Julie's Dream™",
    "number": "26",
    "lightness": 73,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Roman Plaster™",
          "Masquerade™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Purple Brown™"
        ]
      }
    ],
    "color": "#F1E7DD",
    "hsl": {
      "h": 30,
      "s": 41.7,
      "l": 90.6
    },
    "categories": [
      "Neutral",
      "White",
      "Red & Pink",
      "Colours of England",
      "1950s"
    ]
  },
  {
    "title": "Silent White - Mid™",
    "number": "330",
    "lightness": 73,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Travertine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Gold™",
          "Middle Buff™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#F2ECD7",
    "hsl": {
      "h": 47,
      "s": 50.9,
      "l": 89.6
    },
    "categories": [
      "Neutral",
      "White",
      "Colours of England",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Silent White - Mid™"
    ]
  },
  {
    "title": "Clay - Pale™",
    "number": "152",
    "lightness": 78,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Mushroom™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Tuscan Red™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Scullery"
        ]
      }
    ],
    "color": "#EEE8D8",
    "hsl": {
      "h": 44,
      "s": 39.3,
      "l": 89
    },
    "categories": [
      "Neutral",
      "White",
      "Stone"
    ]
  },
  {
    "title": "Clay - Mid™",
    "number": "153",
    "lightness": 69,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Clay - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mushroom™",
          "Lute™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Tuscan Red™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Purple Brown™"
        ]
      }
    ],
    "color": "#E7DDC7",
    "hsl": {
      "h": 41,
      "s": 40,
      "l": 84.3
    },
    "categories": [
      "Neutral",
      "White",
      "Stone"
    ]
  },
  {
    "title": "Travertine - Light™",
    "number": "272",
    "lightness": 68,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bath Stone®",
          "Travertine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Yellow-Pink™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Boxington®"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#F0E4CA",
    "hsl": {
      "h": 41,
      "s": 55.9,
      "l": 86.7
    },
    "categories": [
      "Neutral",
      "White",
      "Stone"
    ]
  },
  {
    "title": "Silent White - Deep™",
    "number": "331",
    "lightness": 67,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Silent White - Pale™",
          "Silent White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bath Stone®",
          "Lute™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Yellow-Pink™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sage Green™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#EBE3C8",
    "hsl": {
      "h": 46,
      "s": 46.7,
      "l": 85.3
    },
    "categories": [
      "Neutral",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Silent White - Deep™"
    ]
  },
  {
    "title": "Stone-Pale-Warm™",
    "number": "34",
    "lightness": 67,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Stock®",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Silt®",
          "Rolling Fog®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Peachblossom™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#F3D5AF",
    "hsl": {
      "h": 34,
      "s": 73.9,
      "l": 82
    },
    "categories": [
      "Neutral",
      "Colours of England",
      "Georgian"
    ]
  },
  {
    "title": "Travertine - Mid™",
    "number": "273",
    "lightness": 60,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Travertine - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Yellow-Pink™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#E6D7B8",
    "hsl": {
      "h": 40,
      "s": 47.9,
      "l": 81.2
    },
    "categories": [
      "Neutral",
      "Stone"
    ]
  },
  {
    "title": "Mushroom™",
    "number": "142",
    "lightness": 51,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Rolling Fog - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Silt®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Tuscan Red™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#D3C3AE",
    "hsl": {
      "h": 34,
      "s": 29.6,
      "l": 75.5
    },
    "categories": [
      "Neutral",
      "Colours of England"
    ]
  },
  {
    "title": "Clay™",
    "number": "39",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Clay - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay - Mid™",
          "Joanna®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Jewel Beetle™",
          "Scullery"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Purple Brown™"
        ]
      }
    ],
    "color": "#D4C59C",
    "hsl": {
      "h": 44,
      "s": 39.4,
      "l": 72.2
    },
    "categories": [
      "Neutral",
      "Colours of England",
      "Stone",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Clay™"
    ]
  },
  {
    "title": "Travertine™",
    "number": "319",
    "lightness": 53,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Travertine - Light™",
          "Travertine - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Bronze Green™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#DFCEB1",
    "hsl": {
      "h": 38,
      "s": 41.8,
      "l": 78.4
    },
    "categories": [
      "Neutral",
      "Colours of England",
      "Stone",
      "1950s"
    ]
  },
  {
    "title": "Hammock™",
    "number": "38",
    "lightness": 60,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Roman Plaster™",
          "Joanna®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marine Blue™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Purple Brown™"
        ]
      }
    ],
    "color": "#DDC8AC",
    "hsl": {
      "h": 34,
      "s": 41.9,
      "l": 77.1
    },
    "categories": [
      "Neutral",
      "Colours of England"
    ]
  },
  {
    "title": "Lute™",
    "number": "317",
    "lightness": 43,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay - Mid™",
          "Joanna®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Olive Colour™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#CAB49C",
    "hsl": {
      "h": 31,
      "s": 30.3,
      "l": 70.2
    },
    "categories": [
      "Neutral",
      "Stone"
    ]
  },
  {
    "title": "Bath Stone®",
    "number": "64",
    "lightness": 43,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Travertine - Light™",
          "Travertine - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bassoon™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Olive Colour™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#CDB684",
    "hsl": {
      "h": 41,
      "s": 42.2,
      "l": 66.1
    },
    "categories": [
      "Neutral",
      "Colours of England",
      "Re:mix Flat Emulsion Paint",
      "Victorian"
    ],
    "aliases": [
      "Re:mix Bath Stone®"
    ]
  },
  {
    "title": "Roman Plaster™",
    "number": "31",
    "lightness": 38,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Linen Wash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hammock™",
          "Joanna®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Blush™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Adventurer™"
        ]
      }
    ],
    "color": "#BFB08F",
    "hsl": {
      "h": 41,
      "s": 27.3,
      "l": 65.5
    },
    "categories": [
      "Neutral",
      "Colours of England"
    ]
  },
  {
    "title": "Slaked Lime™",
    "number": "105",
    "lightness": 82,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Slaked Lime - Deep™",
          "Slaked Lime - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Bone China Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Scree™"
        ]
      }
    ],
    "color": "#F0F1EC",
    "hsl": {
      "h": 72,
      "s": 15.2,
      "l": 93.5
    },
    "categories": [
      "Neutral",
      "White",
      "Colours of England"
    ]
  },
  {
    "title": "French Grey - Pale™",
    "number": "161",
    "lightness": 73,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "French Grey - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pale Wedgwood™",
          "Tracery II™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Hellebore™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#E7E7DB",
    "hsl": {
      "h": 60,
      "s": 20,
      "l": 88.2
    },
    "categories": [
      "Neutral",
      "White",
      "Grey"
    ]
  },
  {
    "title": "Portland Stone - Pale™",
    "number": "155",
    "lightness": 75,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone - Dark™",
          "Baluster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Normandy Grey™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#E9E6D5",
    "hsl": {
      "h": 51,
      "s": 31.2,
      "l": 87.5
    },
    "categories": [
      "Neutral",
      "White",
      "Grey",
      "Stone"
    ]
  },
  {
    "title": "Rolling Fog - Pale™",
    "number": "158",
    "lightness": 76,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Rolling Fog®",
          "Silt®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Attic II™"
        ]
      }
    ],
    "color": "#EEE8D7",
    "hsl": {
      "h": 44,
      "s": 40.4,
      "l": 88.8
    },
    "categories": [
      "Neutral",
      "White",
      "Stone"
    ]
  },
  {
    "title": "Slaked Lime - Mid™",
    "number": "149",
    "lightness": 74,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "Slaked Lime - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Sky Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Tuscan Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Vulcan™"
        ]
      }
    ],
    "color": "#EAECE5",
    "hsl": {
      "h": 77,
      "s": 15.6,
      "l": 91.2
    },
    "categories": [
      "Neutral",
      "White",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Slaked Lime - Mid™"
    ]
  },
  {
    "title": "French Grey - Mid™",
    "number": "162",
    "lightness": 66,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "French Grey - Pale™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "Baluster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Peachblossom™",
          "Windmill Lane™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Hellebore™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#D6D6CA",
    "hsl": {
      "h": 60,
      "s": 12.8,
      "l": 81.6
    },
    "categories": [
      "Neutral",
      "White",
      "Grey"
    ]
  },
  {
    "title": "Portland Stone - Light™",
    "number": "281",
    "lightness": 65,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Portland Stone - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone - Dark™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Livid™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Dark Brunswick Green™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#E4E2D5",
    "hsl": {
      "h": 52,
      "s": 21.7,
      "l": 86.5
    },
    "categories": [
      "Neutral",
      "Grey",
      "Stone"
    ]
  },
  {
    "title": "Rolling Fog - Light™",
    "number": "271",
    "lightness": 62,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Rolling Fog - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Rolling Fog - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Attic II™"
        ]
      }
    ],
    "color": "#E1DCD2",
    "hsl": {
      "h": 40,
      "s": 20,
      "l": 85.3
    },
    "categories": [
      "Neutral",
      "Stone"
    ]
  },
  {
    "title": "French Grey™",
    "number": "113",
    "lightness": 55,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "French Grey - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey - Dark™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Sage Green™",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Tuscan Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#C8C5B7",
    "hsl": {
      "h": 49,
      "s": 13.4,
      "l": 75.1
    },
    "categories": [
      "Neutral",
      "Grey",
      "Colours of England",
      "Re:mix Flat Emulsion Paint",
      "Victorian"
    ],
    "aliases": [
      "Re:mix French Grey™"
    ]
  },
  {
    "title": "Portland Stone™",
    "number": "77",
    "lightness": 57,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Portland Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone - Dark™",
          "Baluster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Livid™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Jewel Beetle™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#C9C5A8",
    "hsl": {
      "h": 53,
      "s": 23.4,
      "l": 72.4
    },
    "categories": [
      "Neutral",
      "Grey",
      "Colours of England",
      "Stone",
      "Re:mix Flat Emulsion Paint",
      "Victorian"
    ],
    "aliases": [
      "Re:mix Portland Stone™"
    ]
  },
  {
    "title": "Rolling Fog®",
    "number": "143",
    "lightness": 49,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Rolling Fog - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Rolling Fog - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Adventurer™",
          "Blush™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Attic II™"
        ]
      }
    ],
    "color": "#C5BBA8",
    "hsl": {
      "h": 39,
      "s": 20,
      "l": 71.6
    },
    "categories": [
      "Neutral",
      "Colours of England",
      "Stone",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Rolling Fog®"
    ]
  },
  {
    "title": "Slaked Lime - Deep™",
    "number": "150",
    "lightness": 61,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "Baluster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Normandy Grey™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Vulcan™"
        ]
      }
    ],
    "color": "#D2C7AE",
    "hsl": {
      "h": 42,
      "s": 28.6,
      "l": 75.3
    },
    "categories": [
      "Neutral"
    ]
  },
  {
    "title": "Slaked Lime - Dark™",
    "number": "151",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "Baluster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Livid™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Toad™"
        ]
      }
    ],
    "color": "#BDB29A",
    "hsl": {
      "h": 41,
      "s": 21,
      "l": 67.3
    },
    "categories": [
      "Neutral"
    ]
  },
  {
    "title": "French Grey - Dark™",
    "number": "163",
    "lightness": 41,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "French Grey - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Ceviche™",
          "French Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Jewel Beetle™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#B4B1A0",
    "hsl": {
      "h": 51,
      "s": 11.8,
      "l": 66.7
    },
    "categories": [
      "Neutral",
      "Grey"
    ]
  },
  {
    "title": "Portland Stone - Dark™",
    "number": "157",
    "lightness": 45,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Portland Stone - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone™",
          "Ceviche™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Atomic Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#A29C7C",
    "hsl": {
      "h": 51,
      "s": 17,
      "l": 56.1
    },
    "categories": [
      "Neutral",
      "Grey",
      "Stone"
    ]
  },
  {
    "title": "Rolling Fog - Dark™",
    "number": "160",
    "lightness": 36,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Rolling Fog - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hammock™",
          "Rolling Fog - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Nether Red™",
          "Arras™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#B1A48A",
    "hsl": {
      "h": 40,
      "s": 20,
      "l": 61.8
    },
    "categories": [
      "Neutral",
      "Stone"
    ]
  },
  {
    "title": "Silt®",
    "number": "40",
    "lightness": 27,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Hollyhock™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Fescue™",
          "Rolling Fog - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hellebore™",
          "Castell Pink™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Tivoli®"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#857E6C",
    "hsl": {
      "h": 43,
      "s": 10.4,
      "l": 47.3
    },
    "categories": [
      "Neutral",
      "Grey",
      "Colours of England"
    ]
  },
  {
    "title": "Pearl Colour - Pale™",
    "number": "167",
    "lightness": 79,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "North Brink Grey™",
          "Pearl Colour - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine™",
          "Salix™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#E6EBE1",
    "hsl": {
      "h": 90,
      "s": 20,
      "l": 90.2
    },
    "categories": [
      "Neutral",
      "White",
      "Green",
      "Grey"
    ]
  },
  {
    "title": "Green Stone - Pale™",
    "number": "268",
    "lightness": 74,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone - Dark™",
          "Green Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boringdon Green™",
          "Kitchen Green®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dark Brunswick Green™"
        ]
      }
    ],
    "color": "#EFEBDD",
    "hsl": {
      "h": 47,
      "s": 36,
      "l": 90.2
    },
    "categories": [
      "Neutral",
      "White",
      "Green",
      "Stone"
    ]
  },
  {
    "title": "Pearl Colour - Mid™",
    "number": "168",
    "lightness": 73,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Pearl Colour - Dark™",
          "Slaked Lime - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine - Deep™",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#DDE3D6",
    "hsl": {
      "h": 88,
      "s": 18.8,
      "l": 86.5
    },
    "categories": [
      "Neutral",
      "White",
      "Green",
      "Grey"
    ]
  },
  {
    "title": "Pearl Colour™",
    "number": "100",
    "lightness": 62,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pearl Colour - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Wood Ash™",
          "Pearl Colour - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine - Deep™",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Tuscan Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#D4DACC",
    "hsl": {
      "h": 86,
      "s": 15.9,
      "l": 82.7
    },
    "categories": [
      "Neutral",
      "Green",
      "Grey",
      "Colours of England",
      "Georgian"
    ]
  },
  {
    "title": "Pearl Colour - Dark™",
    "number": "169",
    "lightness": 50,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pearl Colour - Pale™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Mirror™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ambleside™",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#BEC5B8",
    "hsl": {
      "h": 92,
      "s": 10.1,
      "l": 74.7
    },
    "categories": [
      "Neutral",
      "Grey"
    ]
  },
  {
    "title": "Green Stone - Light™",
    "number": "269",
    "lightness": 63,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "Green Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Blush™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dark Brunswick Green™"
        ]
      }
    ],
    "color": "#E1DBC6",
    "hsl": {
      "h": 47,
      "s": 31,
      "l": 82.9
    },
    "categories": [
      "Neutral",
      "White",
      "Green",
      "Colours of England",
      "Stone"
    ]
  },
  {
    "title": "Green Stone™",
    "number": "270",
    "lightness": 53,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Ceviche™",
          "Green Stone - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Jewel Beetle™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Blush™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dark Brunswick Green™"
        ]
      }
    ],
    "color": "#D2CEB8",
    "hsl": {
      "h": 51,
      "s": 22.4,
      "l": 77.3
    },
    "categories": [
      "Neutral",
      "Green",
      "Stone"
    ]
  },
  {
    "title": "Tracery II™",
    "number": "78",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Ceviche™",
          "Portland Stone - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Sage Green™",
          "Silt®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Tuscan Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#B7B79F",
    "hsl": {
      "h": 60,
      "s": 14.3,
      "l": 67.1
    },
    "categories": [
      "Neutral",
      "Green",
      "Grey",
      "Colours of England",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Tracery II™"
    ]
  },
  {
    "title": "Book Room Green™",
    "number": "322",
    "lightness": 43,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone™",
          "Green Stone - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dark Brunswick Green™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Air Force Blue™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#C2BCA2",
    "hsl": {
      "h": 49,
      "s": 20.8,
      "l": 69.8
    },
    "categories": [
      "Neutral",
      "Green",
      "Stone",
      "Georgian"
    ]
  },
  {
    "title": "Normandy Grey™",
    "number": "79",
    "lightness": 39,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Fescue™",
          "Mirror™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Livid™",
          "Dark Brunswick Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Green Verditer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#A0A07D",
    "hsl": {
      "h": 60,
      "s": 15.6,
      "l": 55.9
    },
    "categories": [
      "Neutral",
      "Green",
      "Grey",
      "Colours of England",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Normandy Grey™"
    ]
  },
  {
    "title": "Ferdinand™",
    "number": "313",
    "lightness": 73,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Beauvais Lilac™",
          "Lute™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Tuscan Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Arras™"
        ]
      }
    ],
    "color": "#FBF1E7",
    "hsl": {
      "h": 30,
      "s": 71.4,
      "l": 94.5
    },
    "categories": [
      "Neutral",
      "White",
      "Red & Pink",
      "Stone",
      "Victorian"
    ]
  },
  {
    "title": "China Clay™",
    "number": "1",
    "lightness": 78,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Rubine Ashes™",
          "China Clay - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Confetti™",
          "Light Peachblossom™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Adventurer™"
        ]
      }
    ],
    "color": "#F7EDE0",
    "hsl": {
      "h": 34,
      "s": 59,
      "l": 92.4
    },
    "categories": [
      "Neutral",
      "White",
      "Colours of England"
    ],
    "aliases": [
      "China Clay™ "
    ]
  },
  {
    "title": "Light Beauvais™",
    "number": "323",
    "lightness": 65,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Roman Plaster™",
          "Castell Pink™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Purple Brown™"
        ]
      }
    ],
    "color": "#F1E0CC",
    "hsl": {
      "h": 32,
      "s": 56.9,
      "l": 87.3
    },
    "categories": [
      "Neutral",
      "White",
      "Red & Pink",
      "Stone",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Light Beauvais™"
    ]
  },
  {
    "title": "China Clay - Mid™",
    "number": "176",
    "lightness": 69,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Perennial Grey™",
          "Mid Lead Colour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Masquerade™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#ECD9CB",
    "hsl": {
      "h": 25,
      "s": 46.5,
      "l": 86.1
    },
    "categories": [
      "Neutral",
      "White"
    ]
  },
  {
    "title": "Beauvais Lilac™",
    "number": "29",
    "lightness": 59,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Julie's Dream™",
          "Roman Plaster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Arras™"
        ]
      }
    ],
    "color": "#E9DAC3",
    "hsl": {
      "h": 36,
      "s": 46.3,
      "l": 83.9
    },
    "categories": [
      "Neutral",
      "Colours of England",
      "Stone",
      "1930s"
    ]
  },
  {
    "title": "China Clay - Deep™",
    "number": "177",
    "lightness": 61,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Perennial Grey™",
          "Arquerite™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Blush™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#DAC2B4",
    "hsl": {
      "h": 22,
      "s": 33.9,
      "l": 78
    },
    "categories": [
      "Neutral"
    ]
  },
  {
    "title": "Castell Pink™",
    "number": "314",
    "lightness": 50,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Ferdinand™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Rolling Fog - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Nether Red™",
          "Arras™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Purple Brown™"
        ]
      }
    ],
    "color": "#E0C8B2",
    "hsl": {
      "h": 29,
      "s": 42.6,
      "l": 78.8
    },
    "categories": [
      "Neutral",
      "Red & Pink",
      "Stone"
    ]
  },
  {
    "title": "China Clay - Dark™",
    "number": "178",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Arquerite™",
          "Rubine Ashes™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Adventurer™",
          "Ashes of Roses™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#CBB1A2",
    "hsl": {
      "h": 22,
      "s": 28.3,
      "l": 71.6
    },
    "categories": [
      "Neutral"
    ]
  },
  {
    "title": "Dorchester Pink®",
    "number": "213",
    "lightness": 53,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Urbane Grey™",
          "Joanna®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Adventurer™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Vulcan™"
        ]
      }
    ],
    "color": "#DCC6BA",
    "hsl": {
      "h": 21,
      "s": 32.7,
      "l": 79.6
    },
    "categories": [
      "Neutral",
      "Red & Pink",
      "Colours of England",
      "1960s"
    ]
  },
  {
    "title": "Baluster™",
    "number": "321",
    "lightness": 26,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Portland Stone - Pale™",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "Portland Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Olive Colour™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#89847E",
    "hsl": {
      "h": 33,
      "s": 4.5,
      "l": 51.6
    },
    "categories": [
      "Neutral",
      "Grey",
      "Stone"
    ]
  },
  {
    "title": "Attic II™",
    "number": "144",
    "lightness": 15,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Rolling Fog - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Rolling Fog - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine™",
          "Hellebore™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sky Blue™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#5A5247",
    "hsl": {
      "h": 35,
      "s": 11.8,
      "l": 31.6
    },
    "categories": [
      "Neutral",
      "Grey",
      "Stone"
    ]
  },
  {
    "title": "Vulcan™",
    "number": "324",
    "lightness": 10,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "French Grey - Pale™",
          "Portland Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dash of Soot™",
          "French Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hellebore™",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#525653",
    "hsl": {
      "h": 135,
      "s": 2.4,
      "l": 32.9
    },
    "categories": [
      "Neutral",
      "Grey"
    ]
  },
  {
    "title": "Nether Red™",
    "number": "315",
    "lightness": 20,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Ferdinand™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey - Dark™",
          "Mushroom™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Peachblossom™",
          "Arras™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#92756F",
    "hsl": {
      "h": 10,
      "s": 13.8,
      "l": 50.4
    },
    "categories": [
      "Neutral",
      "Red & Pink",
      "Colours of England",
      "Stone",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Nether Red™"
    ]
  },
  {
    "title": "Scullery",
    "number": "318",
    "lightness": 13,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Clay - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hellebore™",
          "Masquerade™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Purple Brown™"
        ]
      }
    ],
    "color": "#624B3B",
    "hsl": {
      "h": 25,
      "s": 24.8,
      "l": 30.8
    },
    "categories": [
      "Neutral",
      "Stone",
      "Georgian"
    ]
  },
  {
    "title": "Purple Brown™",
    "number": "8",
    "lightness": 6,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Joanna®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mushroom™",
          "Roman Plaster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Light Peachblossom™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#271410",
    "hsl": {
      "h": 10,
      "s": 41.8,
      "l": 10.8
    },
    "categories": [
      "Neutral",
      "Black",
      "Stone",
      "Victorian"
    ]
  },
  {
    "title": "Light Bronze Green™",
    "number": "123",
    "lightness": 14,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Travertine - Mid™",
          "Travertine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Nether Red™",
          "Book Room Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Giallo™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#5A4620",
    "hsl": {
      "h": 39,
      "s": 47.5,
      "l": 23.9
    },
    "categories": [
      "Neutral",
      "Green",
      "Archive",
      "Colours of England",
      "Stone",
      "Victorian"
    ]
  },
  {
    "title": "Pompeian Ash™",
    "number": "293",
    "lightness": 12,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Portland Stone - Light™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone - Dark™",
          "Baluster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pale Lime™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#5A5F55",
    "hsl": {
      "h": 90,
      "s": 5.6,
      "l": 35.3
    },
    "categories": [
      "Neutral",
      "Green",
      "Grey",
      "Stone",
      "Victorian"
    ]
  },
  {
    "title": "Elysian Ground™",
    "number": "320",
    "lightness": 7,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Travertine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Bronze Green™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#443A2E",
    "hsl": {
      "h": 33,
      "s": 19.3,
      "l": 22.4
    },
    "categories": [
      "Neutral",
      "Black",
      "Grey",
      "Stone"
    ]
  },
  {
    "title": "Chocolate Colour™",
    "number": "124",
    "lightness": 6,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Rolling Fog - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Rolling Fog®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Tuscan Red™",
          "Arras™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#1E160E",
    "hsl": {
      "h": 30,
      "s": 36.4,
      "l": 8.6
    },
    "categories": [
      "Neutral",
      "Black",
      "Stone",
      "Georgian"
    ]
  },
  {
    "title": "Whitening™",
    "number": "41",
    "lightness": 80,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Shirting®",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Travertine - Mid™",
          "Travertine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Gold™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#F4F2E5",
    "hsl": {
      "h": 52,
      "s": 40.5,
      "l": 92.7
    },
    "categories": [
      "White",
      "Colours of England",
      "Georgian"
    ]
  },
  {
    "title": "Down™",
    "number": "242",
    "lightness": 75,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Dark™",
          "Perennial Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Peachblossom™",
          "Ashes of Roses™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#EEE6DD",
    "hsl": {
      "h": 32,
      "s": 33.3,
      "l": 90
    },
    "categories": [
      "White",
      "Grey"
    ]
  },
  {
    "title": "Loft White™",
    "number": "222",
    "lightness": 92,
    "connections": [
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Shallows™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Tracery II™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Trumpet™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#FDFDFA",
    "hsl": {
      "h": 60,
      "s": 42.9,
      "l": 98.6
    },
    "categories": [
      "White",
      "Colours of England"
    ]
  },
  {
    "title": "Shirting®",
    "number": "129",
    "lightness": 88,
    "connections": [
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Portland Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine™",
          "Silt®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Atomic Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#F8F8F8",
    "hsl": {
      "h": 0,
      "s": 0,
      "l": 97.3
    },
    "categories": [
      "White",
      "Colours of England"
    ]
  },
  {
    "title": "Flint™",
    "number": "236",
    "lightness": 86,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "French Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Peachblossom™",
          "Travertine™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Vulcan™"
        ]
      }
    ],
    "color": "#F9F7F2",
    "hsl": {
      "h": 43,
      "s": 36.8,
      "l": 96.3
    },
    "categories": [
      "White",
      "Grey",
      "Colours of England"
    ]
  },
  {
    "title": "Wood Ash™",
    "number": "229",
    "lightness": 79,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "Pearl Colour - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Serpentine™",
          "Windmill Lane™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#EEEAE1",
    "hsl": {
      "h": 42,
      "s": 27.7,
      "l": 90.8
    },
    "categories": [
      "White",
      "Grey",
      "Colours of England"
    ]
  },
  {
    "title": "Gauze®",
    "number": "106",
    "lightness": 75,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Gauze - Dark™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Juniper Ash®",
          "Obscura™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#EEF0F1",
    "hsl": {
      "h": 200,
      "s": 9.7,
      "l": 93.9
    },
    "categories": [
      "White",
      "Grey",
      "Colours of England"
    ]
  },
  {
    "title": "Aquamarine - Pale™",
    "number": "282",
    "lightness": 77,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Windmill Lane™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ambleside™",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#E9F4ED",
    "hsl": {
      "h": 142,
      "s": 33.3,
      "l": 93.5
    },
    "categories": [
      "White",
      "Green",
      "Blue"
    ]
  },
  {
    "title": "Kitchen Green®",
    "number": "85",
    "lightness": 50,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Portland Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Ceviche™",
          "Joanna®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Scullery",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Grey Stone™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dark Brunswick Green™"
        ]
      }
    ],
    "color": "#C9CB9A",
    "hsl": {
      "h": 62,
      "s": 32,
      "l": 70
    },
    "categories": [
      "Green",
      "Colours of England"
    ]
  },
  {
    "title": "Salix™",
    "number": "99",
    "lightness": 58,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bone China Blue™",
          "Baluster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ambleside™",
          "Mid Azure Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#C2D2C0",
    "hsl": {
      "h": 113,
      "s": 16.7,
      "l": 78.8
    },
    "categories": [
      "Green",
      "Grey",
      "Colours of England"
    ]
  },
  {
    "title": "Apple™",
    "number": "137",
    "lightness": 17,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Green Stone - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Citrine™",
          "Olive Colour™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Woad™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#CBC687",
    "hsl": {
      "h": 56,
      "s": 39.5,
      "l": 66.3
    },
    "categories": [
      "Green",
      "Colours of England"
    ]
  },
  {
    "title": "Windmill Lane™",
    "number": "296",
    "lightness": 29,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Linen Wash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Fescue™",
          "Cool Arbour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Acorn™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Thai Sapphire™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#8E9B8A",
    "hsl": {
      "h": 106,
      "s": 7.8,
      "l": 57.5
    },
    "categories": [
      "Green",
      "Colours of England",
      "Georgian"
    ]
  },
  {
    "title": "Boringdon Green™",
    "number": "295",
    "lightness": 38,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Stock®",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Ceviche™",
          "Green Stone - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#A8AF96",
    "hsl": {
      "h": 77,
      "s": 13.5,
      "l": 63.7
    },
    "categories": [
      "Green",
      "Colours of England",
      "Georgian"
    ]
  },
  {
    "title": "Sage Green™",
    "number": "80",
    "lightness": 23,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Portland Stone - Pale™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Green Stone - Light™",
          "Green Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Lute™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mambo™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#77815C",
    "hsl": {
      "h": 76,
      "s": 16.7,
      "l": 43.3
    },
    "categories": [
      "Green",
      "Colours of England",
      "Stone",
      "Re:mix Flat Emulsion Paint",
      "Victorian"
    ],
    "aliases": [
      "Re:mix Sage Green™"
    ]
  },
  {
    "title": "Livid™",
    "number": "263",
    "lightness": 19,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone™",
          "Fescue™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine™",
          "Celestial Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#6D7D78",
    "hsl": {
      "h": 161,
      "s": 6.8,
      "l": 45.9
    },
    "categories": [
      "Green",
      "Grey",
      "Colours of England",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Livid™"
    ]
  },
  {
    "title": "Ambleside™",
    "number": "304",
    "lightness": 21,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey™",
          "Pearl Colour - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine™",
          "Hicks' Blue®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Pale Lime™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#738478",
    "hsl": {
      "h": 138,
      "s": 6.9,
      "l": 48.4
    },
    "categories": [
      "Green",
      "Grey",
      "Victorian"
    ]
  },
  {
    "title": "Mirror™",
    "number": "219",
    "lightness": 71,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "Baluster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine™",
          "Windmill Lane™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Jewel Beetle™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#E5E4D0",
    "hsl": {
      "h": 57,
      "s": 28.8,
      "l": 85.7
    },
    "categories": [
      "Green",
      "Colours of England"
    ]
  },
  {
    "title": "Aquamarine - Light™",
    "number": "283",
    "lightness": 69,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Pearl Colour - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Windmill Lane™",
          "Aquamarine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ambleside™",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#DBE9DF",
    "hsl": {
      "h": 137,
      "s": 24.1,
      "l": 88.6
    },
    "categories": [
      "Green"
    ]
  },
  {
    "title": "Acorn™",
    "number": "87",
    "lightness": 65,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey - Dark™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine - Deep™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Tuscan Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Hopper™"
        ]
      }
    ],
    "color": "#E0E3C4",
    "hsl": {
      "h": 66,
      "s": 35.6,
      "l": 82.9
    },
    "categories": [
      "Green",
      "Colours of England"
    ]
  },
  {
    "title": "Hidey Hole™",
    "number": "307",
    "lightness": 70,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Pearl Colour - Dark™",
          "Ambleside™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine - Deep™",
          "Canton™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#DEEBDC",
    "hsl": {
      "h": 112,
      "s": 27.3,
      "l": 89.2
    },
    "categories": [
      "Green",
      "Blue",
      "Colours of England",
      "1930s"
    ]
  },
  {
    "title": "Tabernacle™",
    "number": "308",
    "lightness": 54,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey - Dark™",
          "Rolling Fog - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Blush™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Cape Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#BED3BB",
    "hsl": {
      "h": 112,
      "s": 21.4,
      "l": 78
    },
    "categories": [
      "Green",
      "Archive"
    ]
  },
  {
    "title": "Aquamarine - Mid™",
    "number": "284",
    "lightness": 57,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pearl Colour - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Windmill Lane™",
          "Pearl Colour - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ambleside™",
          "Tea with Florence™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#C6D6CA",
    "hsl": {
      "h": 135,
      "s": 16.3,
      "l": 80.8
    },
    "categories": [
      "Green",
      "Colours of England"
    ]
  },
  {
    "title": "Oak Apple™",
    "number": "63",
    "lightness": 49,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Silent White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Green Stone - Light™",
          "Travertine - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Sage Green™",
          "Jewel Beetle™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#CDC185",
    "hsl": {
      "h": 50,
      "s": 41.9,
      "l": 66.3
    },
    "categories": [
      "Green",
      "Yellow"
    ]
  },
  {
    "title": "Boxington®",
    "number": "84",
    "lightness": 38,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "White Lead™",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Slaked Lime - Deep™",
          "Travertine - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Apple™",
          "Light Bronze Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Indian Yellow™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#A8AF61",
    "hsl": {
      "h": 65,
      "s": 32.8,
      "l": 53.3
    },
    "categories": [
      "Green",
      "Colours of England"
    ]
  },
  {
    "title": "Pale Lime™",
    "number": "70",
    "lightness": 48,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone™",
          "Travertine - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Citrine™",
          "Olive Colour™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#C5C951",
    "hsl": {
      "h": 62,
      "s": 52.6,
      "l": 55.3
    },
    "categories": [
      "Green",
      "Colours of England",
      "1950s"
    ]
  },
  {
    "title": "Citrine™",
    "number": "71",
    "lightness": 24,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Portland Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pale Lime™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#777F2D",
    "hsl": {
      "h": 66,
      "s": 47.7,
      "l": 33.7
    },
    "categories": [
      "Green",
      "Colours of England",
      "1950s"
    ]
  },
  {
    "title": "Jewel Beetle™",
    "number": "303",
    "lightness": 11,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Whitening™",
          "Silent White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Lute™",
          "Travertine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#566038",
    "hsl": {
      "h": 75,
      "s": 26.3,
      "l": 29.8
    },
    "categories": [
      "Green",
      "Colours of England"
    ]
  },
  {
    "title": "Olive Colour™",
    "number": "72",
    "lightness": "11.1",
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Silent White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone™",
          "Book Room Green™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Apple™",
          "Citrine™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Trumpet™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#42431F",
    "hsl": {
      "h": 62,
      "s": 36.7,
      "l": 19.2
    },
    "categories": [
      "Green",
      "Colours of England",
      "Georgian"
    ]
  },
  {
    "title": "Aquamarine™",
    "number": "138",
    "lightness": 43,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Aquamarine - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Ceviche™",
          "Mirror™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Silt®",
          "Aquamarine - Deep™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marine Blue™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Harley Green™"
        ]
      }
    ],
    "color": "#A8BBA0",
    "hsl": {
      "h": 102,
      "s": 16.6,
      "l": 68
    },
    "categories": [
      "Green",
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Aquamarine - Deep™",
    "number": "198",
    "lightness": 38,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "Joanna®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pleat™",
          "Ambleside™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#85A486",
    "hsl": {
      "h": 122,
      "s": 14.6,
      "l": 58.2
    },
    "categories": [
      "Green",
      "Blue",
      "Colours of England",
      "1960s",
      "1970s"
    ]
  },
  {
    "title": "Green Verditer™",
    "number": "92",
    "lightness": 46,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Linen Wash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey - Mid™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Harley Green™",
          "Pleat™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Pale Lupin™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#6AC59C",
    "hsl": {
      "h": 153,
      "s": 44,
      "l": 59.4
    },
    "categories": [
      "Green",
      "Colours of England",
      "Regency"
    ],
    "aliases": [
      "Verditer Green"
    ]
  },
  {
    "title": "Canton™",
    "number": "94",
    "lightness": 21,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Bone China Blue - Faint™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine™",
          "Salix™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Baked Cherry®"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#236D6F",
    "hsl": {
      "h": 182,
      "s": 52.1,
      "l": 28.6
    },
    "categories": [
      "Green",
      "Blue",
      "Colours of England",
      "1950s"
    ]
  },
  {
    "title": "Tea with Florence™",
    "number": "310",
    "lightness": 18,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pearl Colour - Pale™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Fescue™",
          "Cool Arbour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Harley Green™",
          "Celestial Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Bassoon™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#5C7C7E",
    "hsl": {
      "h": 184,
      "s": 15.6,
      "l": 42.7
    },
    "categories": [
      "Green",
      "Blue",
      "Colours of England",
      "Georgian"
    ]
  },
  {
    "title": "Pleat™",
    "number": "280",
    "lightness": 19,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "Fescue™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Celestial Blue™",
          "Aquamarine - Light™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Bassoon™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#5F7A75",
    "hsl": {
      "h": 169,
      "s": 12.4,
      "l": 42.5
    },
    "categories": [
      "Green"
    ]
  },
  {
    "title": "Goblin™",
    "number": "311",
    "lightness": 11,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Pearl Colour - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Urbane Grey™",
          "Bone China Blue - Faint™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine™",
          "Brighton™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#446562",
    "hsl": {
      "h": 175,
      "s": 19.5,
      "l": 33.1
    },
    "categories": [
      "Green",
      "Colours of England",
      "1930s"
    ]
  },
  {
    "title": "Harley Green™",
    "number": "312",
    "lightness": 10,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Wood Ash™",
          "Pearl Colour - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone - Dark™",
          "Pearl Colour - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Celestial Blue™",
          "Tea with Florence™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Green Verditer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#425958",
    "hsl": {
      "h": 177,
      "s": 14.8,
      "l": 30.4
    },
    "categories": [
      "Green",
      "Blue",
      "Colours of England",
      "1930s"
    ]
  },
  {
    "title": "Mid Azure Green™",
    "number": "96",
    "lightness": 8,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine - Deep™",
          "Royal Navy™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#063025",
    "hsl": {
      "h": 164,
      "s": 77.8,
      "l": 10.6
    },
    "categories": [
      "Green",
      "Colours of England",
      "1930s"
    ]
  },
  {
    "title": "Phthalo Green™",
    "number": "199",
    "lightness": 49,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "Baluster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Canton™",
          "Hopper™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Indian Yellow™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dark Brunswick Green™"
        ]
      }
    ],
    "color": "#74C84C",
    "hsl": {
      "h": 101,
      "s": 53,
      "l": 54.1
    },
    "categories": [
      "Green",
      "Colours of England",
      "1970s"
    ]
  },
  {
    "title": "Pea Green™",
    "number": "91",
    "lightness": 45,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Fescue™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pleat™",
          "Hidey Hole™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#A9C093",
    "hsl": {
      "h": 91,
      "s": 26.3,
      "l": 66.5
    },
    "categories": [
      "Green",
      "Colours of England",
      "Regency"
    ]
  },
  {
    "title": "Garden™",
    "number": "86",
    "lightness": 29,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bath Stone®",
          "Green Stone - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pea Green™",
          "Scullery"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Puck™"
        ]
      }
    ],
    "color": "#739153",
    "hsl": {
      "h": 89,
      "s": 27.2,
      "l": 44.7
    },
    "categories": [
      "Green",
      "Colours of England"
    ]
  },
  {
    "title": "Hopper™",
    "number": "297",
    "lightness": 14,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Green Stone - Pale™",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Portland Stone - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Acorn™",
          "Garden™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#5A6E48",
    "hsl": {
      "h": 92,
      "s": 20.9,
      "l": 35.7
    },
    "categories": [
      "Green",
      "Colours of England",
      "Victorian"
    ]
  },
  {
    "title": "Puck™",
    "number": "298",
    "lightness": 8,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime - Mid™",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "Green Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine - Deep™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#33523A",
    "hsl": {
      "h": 134,
      "s": 23.3,
      "l": 26.1
    },
    "categories": [
      "Green",
      "Colours of England"
    ]
  },
  {
    "title": "Three Farm Green™",
    "number": "306",
    "lightness": 10,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "French Grey - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Canton™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Boxington®"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#475953",
    "hsl": {
      "h": 160,
      "s": 11.2,
      "l": 31.4
    },
    "categories": [
      "Green",
      "Victorian"
    ]
  },
  {
    "title": "Dark Brunswick Green™",
    "number": "88",
    "lightness": 8,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Green Stone™",
          "Portland Stone - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine - Deep™",
          "Pea Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#082F14",
    "hsl": {
      "h": 138,
      "s": 70.9,
      "l": 10.8
    },
    "categories": [
      "Green",
      "Black",
      "Stone",
      "Victorian"
    ]
  },
  {
    "title": "Obsidian Green™",
    "number": "216",
    "lightness": 6,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Wood Ash™",
          "Green Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Green Stone™",
          "Cool Arbour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hopper™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Theatre Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#132019",
    "hsl": {
      "h": 148,
      "s": 25.5,
      "l": 10
    },
    "categories": [
      "Green",
      "Black",
      "Colours of England",
      "1970s"
    ]
  },
  {
    "title": "Invisible Green™",
    "number": "56",
    "lightness": 9,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Slaked Lime - Mid™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone - Dark™",
          "Book Room Green™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Gold™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#2E2B14",
    "hsl": {
      "h": 53,
      "s": 39.4,
      "l": 12.9
    },
    "categories": [
      "Green",
      "Black",
      "Colours of England",
      "Georgian"
    ]
  },
  {
    "title": "Bone China Blue - Pale™",
    "number": "182",
    "lightness": 73,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Urbane Grey™",
          "Bone China Blue™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Grey Stone™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Hellebore™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#E2E6E2",
    "hsl": {
      "h": 120,
      "s": 7.4,
      "l": 89.4
    },
    "categories": [
      "Blue"
    ]
  },
  {
    "title": "Bone China Blue - Mid™",
    "number": "183",
    "lightness": 66,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mid Lead Colour™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Grey Stone™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marine Blue™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#D0D4D0",
    "hsl": {
      "h": 120,
      "s": 4.4,
      "l": 82.4
    },
    "categories": [
      "Blue"
    ]
  },
  {
    "title": "Delicate Blue™",
    "number": "248",
    "lightness": 72,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Urbane Grey™",
          "Bone China Blue™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Etruria™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#DCEDF1",
    "hsl": {
      "h": 191,
      "s": 42.9,
      "l": 90.4
    },
    "categories": [
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Brighton™",
    "number": "203",
    "lightness": 56,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey - Mid™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pleat™",
          "Pale Lime™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#BFD5CB",
    "hsl": {
      "h": 153,
      "s": 20.8,
      "l": 79.2
    },
    "categories": [
      "Blue",
      "Colours of England",
      "1960s"
    ]
  },
  {
    "title": "Air Force Blue™",
    "number": "260",
    "lightness": 21,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Bone China Blue - Faint™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine - Deep™",
          "Brighton™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Light Gold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#558799",
    "hsl": {
      "h": 196,
      "s": 28.6,
      "l": 46.7
    },
    "categories": [
      "Blue",
      "Colours of England",
      "1930s"
    ]
  },
  {
    "title": "Marine Blue™",
    "number": "95",
    "lightness": 13,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Fescue™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Celestial Blue™",
          "Brighton™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#03414E",
    "hsl": {
      "h": 190,
      "s": 92.6,
      "l": 15.9
    },
    "categories": [
      "Blue",
      "Colours of England",
      "1950s"
    ]
  },
  {
    "title": "Pale Wedgwood™",
    "number": "249",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mid Lead Colour™",
          "Shallows™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Juniper Ash®",
          "Pale Lupin™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Arquerite™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#B7CCDA",
    "hsl": {
      "h": 204,
      "s": 32.1,
      "l": 78.6
    },
    "categories": [
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Bone China Blue - Faint™",
    "number": "325",
    "lightness": 54,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bone China Blue - Pale™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Grey Stone™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#C2D0D2",
    "hsl": {
      "h": 187,
      "s": 15.1,
      "l": 79.2
    },
    "categories": [
      "Blue"
    ]
  },
  {
    "title": "Bone China Blue™",
    "number": "107",
    "lightness": 39,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Bone China Blue - Pale™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Bone China Blue - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Scree™",
          "Etruria™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#A8B9BD",
    "hsl": {
      "h": 191,
      "s": 13.7,
      "l": 70
    },
    "categories": [
      "Blue",
      "Colours of England",
      "Re:mix Flat Emulsion Paint",
      "1930s"
    ],
    "aliases": [
      "Re:mix Bone China Blue™"
    ]
  },
  {
    "title": "Celestial Blue™",
    "number": "101",
    "lightness": 46,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey - Mid™",
          "Bone China Blue - Faint™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Livid™",
          "Marine Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Bronze Red®"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Harley Green™"
        ]
      }
    ],
    "color": "#A4B4AF",
    "hsl": {
      "h": 161,
      "s": 9.6,
      "l": 67.5
    },
    "categories": [
      "Blue",
      "Colours of England",
      "Regency"
    ]
  },
  {
    "title": "James™",
    "number": "108",
    "lightness": 37,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "French Grey - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pale Lupin™",
          "Arquerite™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#8597A3",
    "hsl": {
      "h": 204,
      "s": 14,
      "l": 58
    },
    "categories": [
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Grey Stone™",
    "number": "276",
    "lightness": 35,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Wood Ash™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Bone China Blue - Faint™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Heat™",
          "Etruria™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Light Peachblossom™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Scree™"
        ]
      }
    ],
    "color": "#8AA0AD",
    "hsl": {
      "h": 202,
      "s": 17.6,
      "l": 61
    },
    "categories": [
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Pale Lupin™",
    "number": "278",
    "lightness": 20,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Obscura™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Mischief™",
          "Pale Wedgwood™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Green Verditer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#6C7892",
    "hsl": {
      "h": 221,
      "s": 15,
      "l": 49.8
    },
    "categories": [
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Juniper Ash®",
    "number": "115",
    "lightness": 17,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Linen Wash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "Obscura™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pale Wedgwood™",
          "Bone China Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Atomic Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#525969",
    "hsl": {
      "h": 222,
      "s": 12.3,
      "l": 36.7
    },
    "categories": [
      "Blue",
      "Colours of England",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Juniper Ash®"
    ]
  },
  {
    "title": "Etruria™",
    "number": "326",
    "lightness": 19,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pearl Colour - Pale™",
          "Slaked Lime - Mid™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey™",
          "Portland Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bone China Blue™",
          "Brighton™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Bronze Red®"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Harley Green™"
        ]
      }
    ],
    "color": "#627D85",
    "hsl": {
      "h": 194,
      "s": 15.2,
      "l": 45.3
    },
    "categories": [
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Ultra Blue",
    "number": "264",
    "connections": [],
    "color": "#211ACA",
    "hsl": {
      "h": 242,
      "s": 77.2,
      "l": 44.7
    },
    "categories": [
      "Blue",
      "Archive"
    ],
    "aliases": [
      "Ultra Blue "
    ]
  },
  {
    "title": "Sky Blue™",
    "number": "103",
    "lightness": 47,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime - Mid™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Shallows™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Woad™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#ADD4DE",
    "hsl": {
      "h": 192,
      "s": 42.6,
      "l": 77.5
    },
    "categories": [
      "Blue",
      "Colours of England",
      "Georgian"
    ]
  },
  {
    "title": "Blue Verditer™",
    "number": "104",
    "lightness": 34,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey - Mid™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Bronze Green™",
          "Sky Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Atomic Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#659AB2",
    "hsl": {
      "h": 199,
      "s": 33.3,
      "l": 54.7
    },
    "categories": [
      "Blue",
      "Colours of England",
      "Regency"
    ],
    "aliases": [
      "Verditer Blue"
    ]
  },
  {
    "title": "Tivoli®",
    "number": "206",
    "lightness": 31,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bone China Blue - Mid™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Canton™",
          "Deep Space Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Phthalo Green™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#5C8EA9",
    "hsl": {
      "h": 201,
      "s": 30.9,
      "l": 51.2
    },
    "categories": [
      "Blue",
      "Colours of England",
      "1960s",
      "1970s"
    ]
  },
  {
    "title": "Mazarine™",
    "number": "256",
    "lightness": 9,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Cool Arbour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Bone China Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#235580",
    "hsl": {
      "h": 208,
      "s": 57.1,
      "l": 32
    },
    "categories": [
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Mambo™",
    "number": "112",
    "lightness": 17,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Gauze - Dark™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "James™",
          "Smalt™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Hellebore™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#5371B8",
    "hsl": {
      "h": 222,
      "s": 41.6,
      "l": 52.4
    },
    "categories": [
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Deep Space Blue™",
    "number": "207",
    "lightness": 7,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Grey Stone™",
          "James™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#003B59",
    "hsl": {
      "h": 200,
      "s": 100,
      "l": 17.5
    },
    "categories": [
      "Blue",
      "Colours of England",
      "1960s",
      "1970s"
    ]
  },
  {
    "title": "Hicks' Blue®",
    "number": "208",
    "lightness": 10,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "French Grey - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey™",
          "Obscura™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dorchester Pink®",
          "James™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Green Verditer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#415968",
    "hsl": {
      "h": 203,
      "s": 23.1,
      "l": 33.1
    },
    "categories": [
      "Blue",
      "Colours of England",
      "1960s"
    ]
  },
  {
    "title": "Woad™",
    "number": "251",
    "lightness": 12,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Shirting®",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey - Mid™",
          "Obscura™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Grey Stone™",
          "Tivoli®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sunlight™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#436382",
    "hsl": {
      "h": 210,
      "s": 32,
      "l": 38.6
    },
    "categories": [
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Smalt™",
    "number": "255",
    "lightness": 7,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Gauze - Dark™",
          "Obscura™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Mambo™",
          "Canton™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#29467B",
    "hsl": {
      "h": 219,
      "s": 50,
      "l": 32.2
    },
    "categories": [
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Thai Sapphire™",
    "number": "116",
    "lightness": "4.61",
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Down™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Rubine Ashes™",
          "Arquerite™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pink Slip®",
          "Pale Lupin™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#12042A",
    "hsl": {
      "h": 262,
      "s": 82.6,
      "l": 9
    },
    "categories": [
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Royal Navy™",
    "number": "257",
    "lightness": 6,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "French Grey - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bone China Blue - Faint™",
          "Obscura™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hopper™",
          "Marine Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#294356",
    "hsl": {
      "h": 205,
      "s": 35.4,
      "l": 24.9
    },
    "categories": [
      "Blue",
      "Colours of England"
    ]
  },
  {
    "title": "Dock Blue™",
    "number": "252",
    "lightness": 5,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "French Grey - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Urbane Grey™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bone China Blue™",
          "James™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#2E3742",
    "hsl": {
      "h": 213,
      "s": 17.9,
      "l": 22
    },
    "categories": [
      "Blue",
      "Black",
      "Colours of England"
    ]
  },
  {
    "title": "Basalt®",
    "number": "221",
    "lightness": 5,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "French Grey - Pale™",
          "Slaked Lime - Mid™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Grey Stone™",
          "Juniper Ash®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#0C1B24",
    "hsl": {
      "h": 203,
      "s": 50,
      "l": 9.4
    },
    "categories": [
      "Blue",
      "Black",
      "Colours of England"
    ]
  },
  {
    "title": "Light Peachblossom™",
    "number": "3",
    "lightness": 43,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Rolling Fog - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#C6B2A7",
    "hsl": {
      "h": 21,
      "s": 21.4,
      "l": 71.6
    },
    "categories": [
      "Red & Pink",
      "Colours of England",
      "Regency"
    ]
  },
  {
    "title": "Blush™",
    "number": "267",
    "lightness": 24,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Dark™",
          "Masquerade - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Confetti™",
          "Dorchester Pink®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Thai Sapphire™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#AE8A82",
    "hsl": {
      "h": 11,
      "s": 21.4,
      "l": 59.6
    },
    "categories": [
      "Red & Pink",
      "Colours of England"
    ]
  },
  {
    "title": "Adventurer™",
    "number": "7",
    "lightness": 10,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Rolling Fog - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Deep™",
          "Rolling Fog - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dorchester Pink®",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Purple Brown™"
        ]
      }
    ],
    "color": "#5D3A40",
    "hsl": {
      "h": 350,
      "s": 23.2,
      "l": 29.6
    },
    "categories": [
      "Red & Pink",
      "Colours of England"
    ]
  },
  {
    "title": "Ashes of Roses™",
    "number": "6",
    "lightness": 18,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Hollyhock™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Deep™",
          "Hammock™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Confetti™",
          "Mid Lead Colour™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Grey Stone™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#945E5C",
    "hsl": {
      "h": 2,
      "s": 23.3,
      "l": 47.1
    },
    "categories": [
      "Red & Pink",
      "Colours of England",
      "Victorian"
    ]
  },
  {
    "title": "Tuscan Red™",
    "number": "140",
    "lightness": 17,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Ferdinand™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Slaked Lime - Deep™",
          "French Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Castell Pink™",
          "Arras™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Purple Brown™"
        ]
      }
    ],
    "color": "#874535",
    "hsl": {
      "h": 12,
      "s": 43.6,
      "l": 36.9
    },
    "categories": [
      "Red & Pink",
      "Colours of England"
    ]
  },
  {
    "title": "Arras™",
    "number": "316",
    "lightness": 8,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Ferdinand™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hammock™",
          "Castell Pink™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Rolling Fog - Dark™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Tuscan Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Purple Brown™"
        ]
      }
    ],
    "color": "#744442",
    "hsl": {
      "h": 2,
      "s": 27.5,
      "l": 35.7
    },
    "categories": [
      "Red & Pink",
      "Stone"
    ]
  },
  {
    "title": "Carmine™",
    "number": "189",
    "lightness": 24,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Hollyhock™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey™",
          "Dash of Soot™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pink Slip®",
          "Masquerade™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Adventurer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#D5666C",
    "hsl": {
      "h": 357,
      "s": 56.9,
      "l": 61.8
    },
    "categories": [
      "Red & Pink",
      "Colours of England",
      "1960s"
    ]
  },
  {
    "title": "Leather™",
    "number": "191",
    "lightness": 19,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Down™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Dark™",
          "French Grey - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Arquerite™",
          "Confetti™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Thai Sapphire™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#CC3852",
    "hsl": {
      "h": 349,
      "s": 59.2,
      "l": 51
    },
    "categories": [
      "Red & Pink",
      "Colours of England",
      "1960s",
      "1970s"
    ]
  },
  {
    "title": "Mischief™",
    "number": "13",
    "lightness": 16,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Chemise™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Rubine Ashes™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Arquerite™",
          "Dorchester Pink®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mambo™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#AE385D",
    "hsl": {
      "h": 341,
      "s": 51.3,
      "l": 45.1
    },
    "categories": [
      "Red & Pink",
      "Colours of England"
    ]
  },
  {
    "title": "Orange Aurora™",
    "number": "21",
    "lightness": 29,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Beauvais Lilac™",
          "Joanna®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Atomic Red™",
          "Confetti™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marine Blue™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#E3634B",
    "hsl": {
      "h": 9,
      "s": 73.1,
      "l": 59.2
    },
    "categories": [
      "Red & Pink",
      "Yellow",
      "Colours of England",
      "1950s"
    ]
  },
  {
    "title": "Atomic Red™",
    "number": "190",
    "lightness": 15,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Linen Wash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey™",
          "Rolling Fog®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Orange Aurora™",
          "Woad™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Brighton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#BD2520",
    "hsl": {
      "h": 2,
      "s": 71,
      "l": 43.3
    },
    "categories": [
      "Red & Pink",
      "Colours of England",
      "1960s",
      "1970s"
    ]
  },
  {
    "title": "Heat™",
    "number": "24",
    "lightness": 15,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Slaked Lime - Dark™",
          "Travertine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Adventurer™",
          "Light Gold™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Pleat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#9F3C19",
    "hsl": {
      "h": 16,
      "s": 72.8,
      "l": 36.1
    },
    "categories": [
      "Red & Pink",
      "Yellow",
      "Colours of England"
    ]
  },
  {
    "title": "Baked Cherry®",
    "number": "14",
    "lightness": 9,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Linen Wash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Rolling Fog®",
          "Mushroom™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Invisible Green™",
          "Oak Apple™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#611117",
    "hsl": {
      "h": 356,
      "s": 70.2,
      "l": 22.4
    },
    "categories": [
      "Red & Pink",
      "Colours of England"
    ]
  },
  {
    "title": "Theatre Red™",
    "number": "192",
    "lightness": 9,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey™",
          "Rolling Fog - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Carmine™",
          "Royal Navy™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Giallo™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Purple Brown™"
        ]
      }
    ],
    "color": "#711324",
    "hsl": {
      "h": 349,
      "s": 71.2,
      "l": 25.9
    },
    "categories": [
      "Red & Pink",
      "Colours of England",
      "1970s"
    ]
  },
  {
    "title": "Bronze Red®",
    "number": "15",
    "lightness": 9,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Stock®",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mushroom™",
          "Clay™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Lute™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#6C2322",
    "hsl": {
      "h": 1,
      "s": 52.1,
      "l": 27.8
    },
    "categories": [
      "Red & Pink",
      "Colours of England",
      "Victorian"
    ]
  },
  {
    "id": "cordoba",
    "title": "Córdoba™",
    "number": "277",
    "lightness": 6,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Down™",
          "China Clay™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dash of Soot™",
          "Perennial Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#493B3C",
    "hsl": {
      "h": 356,
      "s": 10.6,
      "l": 25.9
    },
    "categories": [
      "Red & Pink",
      "Black",
      "Colours of England"
    ],
    "aliases": [
      "Cordoba",
      "Córdoba"
    ]
  },
  {
    "title": "Chemise™",
    "number": "139",
    "lightness": 74,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey™",
          "Perennial Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dorchester Pink®",
          "Hellebore™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Adventurer™"
        ]
      }
    ],
    "color": "#F5E8E6",
    "hsl": {
      "h": 8,
      "s": 42.9,
      "l": 93.1
    },
    "categories": [
      "Red & Pink",
      "Colours of England",
      "1950s"
    ]
  },
  {
    "title": "Masquerade - Light™",
    "number": "332",
    "lightness": 64,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Julie's Dream™",
          "Roman Plaster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#ECDBCA",
    "hsl": {
      "h": 30,
      "s": 47.2,
      "l": 85.9
    },
    "categories": [
      "Red & Pink"
    ]
  },
  {
    "title": "Masquerade - Mid™",
    "number": "333",
    "lightness": 56,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Julie's Dream™",
          "Roman Plaster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#E2CCB8",
    "hsl": {
      "h": 29,
      "s": 42,
      "l": 80.4
    },
    "categories": [
      "Red & Pink"
    ]
  },
  {
    "title": "Masquerade™",
    "number": "334",
    "lightness": 45,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Julie's Dream™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Light Beauvais™",
          "Masquerade - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Arras™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Pleat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#D2B6A0",
    "hsl": {
      "h": 26,
      "s": 35.7,
      "l": 72.5
    },
    "categories": [
      "Red & Pink",
      "Colours of England"
    ]
  },
  {
    "title": "Pink Slip®",
    "number": "220",
    "lightness": 63,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Perennial Grey™",
          "Arquerite™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hellebore™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Theatre Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#ECD2CB",
    "hsl": {
      "h": 13,
      "s": 46.5,
      "l": 86.1
    },
    "categories": [
      "Red & Pink",
      "Colours of England",
      "1950s"
    ]
  },
  {
    "title": "Confetti™",
    "number": "274",
    "lightness": 60,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "China Clay™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Perennial Grey™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Arquerite™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#E9D0CF",
    "hsl": {
      "h": 2,
      "s": 37.1,
      "l": 86.3
    },
    "categories": [
      "Red & Pink",
      "Colours of England"
    ]
  },
  {
    "title": "Hellebore™",
    "number": "275",
    "lightness": 39,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Deep™",
          "French Grey - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Carmine™",
          "Pink Slip®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Olive Colour™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#C8A5A3",
    "hsl": {
      "h": 3,
      "s": 25.2,
      "l": 71.2
    },
    "categories": [
      "Red & Pink",
      "Colours of England"
    ]
  },
  {
    "title": "Marigold™",
    "number": "209",
    "lightness": 36,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Whitening™",
          "Silent White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Fescue™",
          "Travertine - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Carmine™",
          "Arras™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#F0841F",
    "hsl": {
      "h": 29,
      "s": 87.4,
      "l": 53.1
    },
    "categories": [
      "Yellow",
      "Colours of England",
      "1960s",
      "1970s"
    ]
  },
  {
    "title": "Middle Buff™",
    "number": "122",
    "lightness": 22,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hammock™",
          "Slaked Lime - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Bronze Red®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marine Blue™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#AF732B",
    "hsl": {
      "h": 33,
      "s": 60.6,
      "l": 42.7
    },
    "categories": [
      "Yellow",
      "Colours of England",
      "1930s"
    ]
  },
  {
    "title": "Carys™",
    "number": "148",
    "lightness": 70,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone - Dark™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Light Gold™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Air Force Blue™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Light Bronze Green™"
        ]
      }
    ],
    "color": "#FFE57B",
    "hsl": {
      "h": 48,
      "s": 100,
      "l": 74.1
    },
    "categories": [
      "Yellow",
      "Colours of England"
    ]
  },
  {
    "title": "Sunlight™",
    "number": "135",
    "lightness": 53,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Ceviche™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Bronze Green™",
          "Bassoon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#DFC774",
    "hsl": {
      "h": 47,
      "s": 62.6,
      "l": 66.5
    },
    "categories": [
      "Yellow",
      "Colours of England",
      "Re:mix Flat Emulsion Paint"
    ],
    "aliases": [
      "Re:mix Sunlight™"
    ]
  },
  {
    "title": "Bassoon™",
    "number": "336",
    "lightness": 35,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Travertine - Mid™",
          "Silent White - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Sunlight™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#C09F66",
    "hsl": {
      "h": 38,
      "s": 41.7,
      "l": 57.6
    },
    "categories": [
      "Yellow",
      "Colours of England"
    ]
  },
  {
    "title": "Indian Yellow™",
    "number": "335",
    "lightness": 59,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Baluster™",
          "Book Room Green™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Citrine™",
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Arras™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dark Brunswick Green™"
        ]
      }
    ],
    "color": "#EDE068",
    "hsl": {
      "h": 54,
      "s": 78.7,
      "l": 66.9
    },
    "categories": [
      "Yellow",
      "Colours of England"
    ]
  },
  {
    "title": "Giallo™",
    "number": "337",
    "lightness": 55,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone™",
          "Portland Stone - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Jewel Beetle™",
          "Middle Buff™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#EED266",
    "hsl": {
      "h": 48,
      "s": 80,
      "l": 66.7
    },
    "categories": [
      "Yellow",
      "Colours of England"
    ]
  },
  {
    "title": "Light Gold™",
    "number": "53",
    "lightness": 48,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "White Lead™",
          "Silent White - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bath Stone®",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Sunlight™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Scree™"
        ]
      }
    ],
    "color": "#D8BB5D",
    "hsl": {
      "h": 46,
      "s": 61.2,
      "l": 60.6
    },
    "categories": [
      "Yellow",
      "Colours of England",
      "Victorian"
    ]
  },
  {
    "title": "Yellow-Pink™",
    "number": "46",
    "lightness": 41,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Silent White - Pale™",
          "Silent White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "Slaked Lime - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Middle Buff™",
          "Olive Colour™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Puck™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#D0A946",
    "hsl": {
      "h": 43,
      "s": 59.5,
      "l": 54.5
    },
    "categories": [
      "Yellow",
      "Colours of England",
      "Re:mix Flat Emulsion Paint",
      "Regency"
    ],
    "aliases": [
      "Re:mix Yellow-Pink™"
    ]
  },
  {
    "title": "Mister David™",
    "number": "47",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "Silt®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Citrine™",
          "Marigold™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Atomic Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Elysian Ground™"
        ]
      }
    ],
    "color": "#E1C022",
    "hsl": {
      "h": 50,
      "s": 76.1,
      "l": 50.8
    },
    "categories": [
      "Yellow",
      "Colours of England"
    ],
    "aliases": [
      "Mr David"
    ]
  },
  {
    "title": "Trumpet™",
    "number": "196",
    "lightness": 65,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Acorn™",
          "Kitchen Green®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pale Lime™",
          "Phthalo Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Scree™"
        ]
      }
    ],
    "color": "#F1F15C",
    "hsl": {
      "h": 60,
      "s": 84.2,
      "l": 65.3
    },
    "categories": [
      "Yellow",
      "Colours of England",
      "1960s"
    ]
  },
  {
    "title": "Jack Black™",
    "number": "119",
    "lightness": 4,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mid Lead Colour™",
          "French Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Hicks' Blue®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Tivoli®"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#040E0F",
    "hsl": {
      "h": 185,
      "s": 57.9,
      "l": 3.7
    },
    "categories": [
      "Black",
      "Colours of England"
    ]
  },
  {
    "title": "Lamp Black™",
    "number": "228",
    "lightness": 7,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Shallows™",
          "French Grey - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Lead Colour™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#2E2D2E",
    "hsl": {
      "h": 300,
      "s": 1.1,
      "l": 17.8
    },
    "categories": [
      "Black",
      "Grey",
      "Colours of England"
    ]
  },
  {
    "title": "Ceviche™",
    "number": "230",
    "lightness": 70,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Peachblossom™",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#E0DBCF",
    "hsl": {
      "h": 42,
      "s": 21.5,
      "l": 84.5
    },
    "categories": [
      "Grey"
    ]
  },
  {
    "title": "Fescue™",
    "number": "231",
    "lightness": 61,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey - Dark™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Windmill Lane™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Arras™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#CCC7B9",
    "hsl": {
      "h": 44,
      "s": 15.7,
      "l": 76.3
    },
    "categories": [
      "Grey"
    ]
  },
  {
    "title": "Cool Arbour™",
    "number": "232",
    "lightness": 56,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Ceviche™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ambleside™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Puck™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#B1AB9B",
    "hsl": {
      "h": 44,
      "s": 12.4,
      "l": 65.1
    },
    "categories": [
      "Grey"
    ]
  },
  {
    "title": "Serpentine™",
    "number": "233",
    "lightness": 37,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Fescue™",
          "Slaked Lime - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Bronze Green™",
          "Green Stone™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#989180",
    "hsl": {
      "h": 42,
      "s": 10.4,
      "l": 54.9
    },
    "categories": [
      "Grey",
      "Colours of England"
    ]
  },
  {
    "title": "Dark Lead Colour™",
    "number": "118",
    "lightness": 18,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Wood Ash™",
          "Down™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dash of Soot™",
          "Rubine Ashes™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Peachblossom™",
          "Castell Pink™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#6D6A65",
    "hsl": {
      "h": 38,
      "s": 3.8,
      "l": 41.2
    },
    "categories": [
      "Grey",
      "Georgian"
    ]
  },
  {
    "title": "Scree™",
    "number": "227",
    "lightness": 15,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "French Grey - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Urbane Grey™",
          "Shallows™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bone China Blue™",
          "Light Peachblossom™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#595A57",
    "hsl": {
      "h": 80,
      "s": 1.7,
      "l": 34.7
    },
    "categories": [
      "Grey",
      "Colours of England"
    ]
  },
  {
    "title": "Shallows™",
    "number": "223",
    "lightness": 76,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Urbane Grey™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pearl Colour - Dark™",
          "Celestial Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Smalt™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Scree™"
        ]
      }
    ],
    "color": "#F1F1EF",
    "hsl": {
      "h": 60,
      "s": 6.7,
      "l": 94.1
    },
    "categories": [
      "Grey"
    ]
  },
  {
    "title": "Gauze - Mid™",
    "number": "164",
    "lightness": 69,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mid Lead Colour™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Grey Stone™",
          "James™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mambo™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#E4E6E8",
    "hsl": {
      "h": 210,
      "s": 8,
      "l": 90.2
    },
    "categories": [
      "Grey"
    ]
  },
  {
    "title": "Gauze - Dark™",
    "number": "166",
    "lightness": 56,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mid Lead Colour™",
          "Scree™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Grey Stone™",
          "Pale Lupin™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mambo™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#C7CCCF",
    "hsl": {
      "h": 202,
      "s": 7.7,
      "l": 79.6
    },
    "categories": [
      "Grey"
    ]
  },
  {
    "title": "Inox™",
    "number": "224",
    "lightness": 67,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shirting®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Urbane Grey™",
          "Obscura™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bone China Blue™",
          "Grey Stone™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mambo™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Scree™"
        ]
      }
    ],
    "color": "#DBDBD8",
    "hsl": {
      "h": 60,
      "s": 4,
      "l": 85.3
    },
    "categories": [
      "Grey"
    ]
  },
  {
    "title": "Obscura™",
    "number": "327",
    "lightness": 55,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "French Grey - Pale™",
          "Gauze®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mid Lead Colour™",
          "Scree™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hicks' Blue®",
          "Juniper Ash®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#BEC4C7",
    "hsl": {
      "h": 200,
      "s": 7.4,
      "l": 76.3
    },
    "categories": [
      "Grey"
    ]
  },
  {
    "title": "Urbane Grey™",
    "number": "225",
    "lightness": 42,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shallows™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Scree™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Woad™",
          "Hicks' Blue®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#A1A19C",
    "hsl": {
      "h": 60,
      "s": 2.6,
      "l": 62.2
    },
    "categories": [
      "Grey"
    ]
  },
  {
    "title": "Lead Colour™",
    "number": "117",
    "lightness": 33,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Linen Wash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey™",
          "Rubine Ashes™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dorchester Pink®",
          "Light Peachblossom™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Pale Lime™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#969183",
    "hsl": {
      "h": 44,
      "s": 8.3,
      "l": 55.1
    },
    "categories": [
      "Grey",
      "Colours of England",
      "Georgian"
    ]
  },
  {
    "title": "Mid Lead Colour™",
    "number": "114",
    "lightness": 29,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Scree™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Olive Colour™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Brighton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#8C8C8C",
    "hsl": {
      "h": 0,
      "s": 0,
      "l": 54.9
    },
    "categories": [
      "Grey",
      "Colours of England",
      "Georgian"
    ]
  },
  {
    "title": "Rubine Ashes™",
    "number": "243",
    "lightness": 62,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Down™",
          "China Clay™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Arquerite™",
          "Perennial Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Masquerade™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Hellebore™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#D5CEC8",
    "hsl": {
      "h": 28,
      "s": 13.4,
      "l": 81
    },
    "categories": [
      "Grey"
    ]
  },
  {
    "title": "Dash of Soot™",
    "number": "244",
    "lightness": 53,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Down™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Lead Colour™",
          "Arquerite™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Nether Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#C9C0BA",
    "hsl": {
      "h": 24,
      "s": 12.2,
      "l": 75.9
    },
    "categories": [
      "Grey"
    ]
  },
  {
    "title": "Perennial Grey™",
    "number": "245",
    "lightness": 44,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Down™",
          "China Clay™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Rubine Ashes™",
          "China Clay - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Adventurer™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#AFA49C",
    "hsl": {
      "h": 25,
      "s": 10.6,
      "l": 64.9
    },
    "categories": [
      "Grey"
    ]
  },
  {
    "title": "Arquerite™",
    "number": "250",
    "lightness": 25,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Down™",
          "China Clay™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dash of Soot™",
          "Rubine Ashes™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dorchester Pink®",
          "Hellebore™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#8C8C91",
    "hsl": {
      "h": 240,
      "s": 2.2,
      "l": 55.9
    },
    "categories": [
      "Grey"
    ],
    "aliases": [
      "Arkwright"
    ]
  },
  {
    "title": "Mid Bronze Green™",
    "number": "125",
    "lightness": 10,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Boringdon Green™",
          "Cool Arbour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ulla™",
          "Windmill Lane™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sage & Onions™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#293624",
    "hsl": {
      "h": 103,
      "s": 20,
      "l": 17.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Middle Brown™",
    "lightness": 10,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Hammock™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Cream Colour™",
          "Straw Colour™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#443122",
    "hsl": {
      "h": 26,
      "s": 33.3,
      "l": 20
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Milk Thistle™",
    "number": "187",
    "lightness": 49,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Slaked Lime - Mid™",
          "Welcome™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Chemise™",
          "Dorchester Pink - Mid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Arquerite™"
        ]
      }
    ],
    "color": "#C4B1B9",
    "hsl": {
      "h": 335,
      "s": 13.9,
      "l": 73.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Mirage™",
    "lightness": 75,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Slaked Lime™",
          "Welcome - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Perennial Grey™",
          "Welcome - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hellebore™",
          "Light Peachblossom™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Arquerite™"
        ]
      }
    ],
    "color": "#E2E0E3",
    "hsl": {
      "h": 280,
      "s": 5.1,
      "l": 88.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Mocha™",
    "lightness": 12,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Linen Wash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Mushroom™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Middle Buff™",
          "Mortlake Yellow™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Cork™"
        ]
      }
    ],
    "color": "#62472A",
    "hsl": {
      "h": 31,
      "s": 40,
      "l": 27.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Moon Shadow™",
    "number": "261",
    "lightness": 11,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mirror™",
          "Pearl Colour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Brighton™",
          "Salix™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Green Verditer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#156584",
    "hsl": {
      "h": 197,
      "s": 72.5,
      "l": 30
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Mortar™",
    "number": "239",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Tusk™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Limestone™",
          "True Taupe™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Pleat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#CCC1AB",
    "hsl": {
      "h": 40,
      "s": 24.4,
      "l": 73.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "North Brink Grey™",
    "number": "291",
    "lightness": 39,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Portland Stone - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Ceviche™",
          "Putti™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Canton™",
          "Confetti™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#ABB0A3",
    "hsl": {
      "h": 83,
      "s": 7.6,
      "l": 66.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Old Paper II™",
    "number": "146",
    "lightness": 79,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hammock™",
          "Mushroom™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Peachblossom™",
          "Roman Plaster™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Adventurer™"
        ]
      }
    ],
    "color": "#FFEFDD",
    "hsl": {
      "h": 32,
      "s": 100,
      "l": 93.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Olive Oil™",
    "number": "83",
    "lightness": 60,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Slaked Lime™",
          "Rolling Fog - Pale™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Stone-Dark-Warm™",
          "White Lead - Mid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Olive Colour™"
        ]
      }
    ],
    "color": "#DBD7A7",
    "hsl": {
      "h": 55,
      "s": 41.9,
      "l": 75.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Palatine™",
    "lightness": "12.8",
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Perennial Grey™",
          "Welcome - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Arquerite™",
          "Hortense™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#5A3C79",
    "hsl": {
      "h": 270,
      "s": 33.7,
      "l": 35.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Pale Berlin™",
    "number": "258",
    "lightness": 66,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Aquamarine - Pale™",
          "Celestial White™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Canton™",
          "Old School Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sage & Onions™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Tea with Florence™"
        ]
      }
    ],
    "color": "#BBE9ED",
    "hsl": {
      "h": 185,
      "s": 58.1,
      "l": 83.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Palm Deering™",
    "lightness": 12,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pendula™",
          "Pique™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Fescue™",
          "Ulla™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boringdon Green™",
          "Pea Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Green Verditer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#536559",
    "hsl": {
      "h": 140,
      "s": 9.8,
      "l": 36.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Papyrus™",
    "lightness": 80,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Stock - Dark™",
          "Stock - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Apple™",
          "Oak Apple™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Silt®"
        ]
      }
    ],
    "color": "#FBFCDD",
    "hsl": {
      "h": 62,
      "s": 83.8,
      "l": 92.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Pavilion™",
    "lightness": 77,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hammock™",
          "Roman Plaster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Tuscany™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#FFEFDB",
    "hsl": {
      "h": 33,
      "s": 100,
      "l": 92.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Peep™",
    "lightness": 23,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Gauze®",
          "Welcome - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Gauze - Dark™",
          "Mono™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hortense™",
          "Pale Wedgwood™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#79829F",
    "hsl": {
      "h": 226,
      "s": 16.5,
      "l": 54.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Pendula™",
    "number": "289",
    "lightness": 75,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey™",
          "Portland Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Tabernacle™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Green Verditer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#ECF1EA",
    "hsl": {
      "h": 103,
      "s": 20,
      "l": 93.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Piermint™",
    "lightness": 65,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pearl Colour - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hidey Hole™",
          "Slaked Lime - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ambleside™",
          "Pleat™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Goblin™"
        ]
      }
    ],
    "color": "#C0DBBA",
    "hsl": {
      "h": 109,
      "s": 31.4,
      "l": 79.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Pipedream™",
    "lightness": 34,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Dorchester Pink - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mirage II™",
          "Rubine Ashes™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Confetti™",
          "Pink Slip®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#C3919D",
    "hsl": {
      "h": 346,
      "s": 29.4,
      "l": 66.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Pitcairn™",
    "number": "61",
    "lightness": 66,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Stock®",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Stock - Mid™",
          "Stone-Dark-Cool™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Sir Lutyens’ Sage™",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Olive Colour™"
        ]
      }
    ],
    "color": "#F0DEA9",
    "hsl": {
      "h": 45,
      "s": 70.3,
      "l": 80.2
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Popper™",
    "lightness": 22,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Dorchester Pink - Pale™",
          "Welcome - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dash of Soot™",
          "Welcome - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dorchester Pink - Deep™",
          "Hortense™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#877AA8",
    "hsl": {
      "h": 257,
      "s": 20.9,
      "l": 56.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Pot Red™",
    "lightness": 23,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Rubine Ashes™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Drummond™",
          "Mortlake Yellow™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mandarin™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#B5755A",
    "hsl": {
      "h": 18,
      "s": 38.1,
      "l": 53.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Putti™",
    "number": "292",
    "lightness": 29,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Down™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Fescue™",
          "Limestone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Celestial Blue™",
          "Hellebore™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Cape Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#989988",
    "hsl": {
      "h": 64,
      "s": 7.7,
      "l": 56.7
    },
    "categories": [
      "Archive",
      "Georgian"
    ]
  },
  {
    "title": "Raglan™",
    "lightness": 27,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Limestone™",
          "Portland Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Apple™",
          "Sunlight™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#95906A",
    "hsl": {
      "h": 53,
      "s": 16.9,
      "l": 50
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Rectory White™",
    "lightness": 94,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone - Deep™",
          "Slaked Lime - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Peachblossom™",
          "Roman Plaster™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Knightsbridge™"
        ]
      }
    ],
    "color": "#F8F9F3",
    "hsl": {
      "h": 70,
      "s": 33.3,
      "l": 96.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Regency Fawn™",
    "lightness": 51,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Hollyhock™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Silt®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#D7C8AB",
    "hsl": {
      "h": 40,
      "s": 35.5,
      "l": 75.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Rolling Fog - Mid™",
    "number": "159",
    "lightness": 68,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Hollyhock™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Rolling Fog - Dark™",
          "Silt®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#DBD5C4",
    "hsl": {
      "h": 44,
      "s": 24.2,
      "l": 81.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Rusling™",
    "number": "9",
    "lightness": 82,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Deep™",
          "Roman Plaster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Heat™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Drummond™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Adventurer™"
        ]
      }
    ],
    "color": "#FFF0E0",
    "hsl": {
      "h": 31,
      "s": 100,
      "l": 93.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Sea Heather™",
    "lightness": 35,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Dorchester Pink - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Welcome™",
          "Welcome - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dorchester Pink - Deep™",
          "Hortense™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#B4A1B4",
    "hsl": {
      "h": 300,
      "s": 11.2,
      "l": 66.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Serenity™",
    "lightness": 96,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone™",
          "Rolling Fog®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Drummond™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Attic II™"
        ]
      }
    ],
    "color": "#FFFBEA",
    "hsl": {
      "h": 49,
      "s": 100,
      "l": 95.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Shrimp Pink™",
    "lightness": 55,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Julie's Dream™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Terra di Sienna™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#F7C7A9",
    "hsl": {
      "h": 23,
      "s": 83,
      "l": 81.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Sir Lutyens’ Sage™",
    "number": "302",
    "lightness": 28,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Limestone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Light Gold™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Atomic Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#96956D",
    "hsl": {
      "h": 59,
      "s": 16.3,
      "l": 50.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "id": "4-00",
    "title": "4 AM™",
    "lightness": 45,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Dorchester Pink - Pale™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Deep™",
          "Mirage II™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Livid™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#C9AEA3",
    "hsl": {
      "h": 17,
      "s": 26,
      "l": 71.4
    },
    "categories": [
      "Archive"
    ],
    "aliases": [
      "4AM",
      "Four AM"
    ]
  },
  {
    "title": "Slipper™",
    "lightness": 30,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Celestial White™",
          "Salix™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Green Verditer™",
          "Twist™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Trumpet™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Goblin™"
        ]
      }
    ],
    "color": "#00A79A",
    "hsl": {
      "h": 175,
      "s": 100,
      "l": 32.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Acorn - Mid™",
    "number": "294",
    "lightness": 73,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "Slaked Lime - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Peachblossom™",
          "Pea Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Atomic Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#EAEFDA",
    "hsl": {
      "h": 74,
      "s": 39.6,
      "l": 89.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Spark's Grey™",
    "lightness": 19,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Rolling Fog - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dash of Soot™",
          "French Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dorchester Pink®",
          "Hellebore™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#7E7976",
    "hsl": {
      "h": 22,
      "s": 3.3,
      "l": 47.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Aged Ivory™",
    "number": "131",
    "lightness": 57,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay - Mid™",
          "Stock - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Stone-Dark-Warm™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Drummond™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#E1D1AD",
    "hsl": {
      "h": 42,
      "s": 46.4,
      "l": 78
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Stargazer™",
    "lightness": 79,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Light Gold™",
          "White Lead - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Carys™",
          "Lemon Tree™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Tan™"
        ]
      }
    ],
    "color": "#FEF2B5",
    "hsl": {
      "h": 50,
      "s": 97.3,
      "l": 85.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Angie™",
    "number": "185",
    "lightness": 64,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Dark™",
          "Dash of Soot™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Sir Lutyens’ Sage™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#F8C1AF",
    "hsl": {
      "h": 15,
      "s": 83.9,
      "l": 82.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Stock - Dark™",
    "number": "175",
    "lightness": 60,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "True Taupe™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Middle Buff™",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Toad™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Yellow-Pink™"
        ]
      }
    ],
    "color": "#E0CE9D",
    "hsl": {
      "h": 44,
      "s": 51.9,
      "l": 74.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Basket™",
    "lightness": 50,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Batsman™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Stock - Mid™",
          "White Lead - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Middle Brown™",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#CCB99B",
    "hsl": {
      "h": 37,
      "s": 32.5,
      "l": 70.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Stock - Deep™",
    "number": "174",
    "lightness": 68,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mushroom™",
          "True Taupe™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Mortlake Yellow™",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Toad™"
        ]
      }
    ],
    "color": "#E9DDBC",
    "hsl": {
      "h": 44,
      "s": 50.6,
      "l": 82.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Batsman™",
    "lightness": 80,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Acre™",
          "Stock - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Chamois™",
          "Stone-Pale-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Furrow™"
        ]
      }
    ],
    "color": "#F6F6EE",
    "hsl": {
      "h": 60,
      "s": 30.8,
      "l": 94.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Stone-Dark-Cool™",
    "number": "67",
    "lightness": 46,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Stock®",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay - Mid™",
          "Joanna®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Drummond™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Toad™"
        ]
      }
    ],
    "color": "#C7BE9C",
    "hsl": {
      "h": 47,
      "s": 27.7,
      "l": 69.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Bertie's Best™",
    "lightness": 40,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Dorchester Pink - Pale™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Deep™",
          "Mirage II™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Carmine™",
          "Confetti™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#E09599",
    "hsl": {
      "h": 357,
      "s": 54.7,
      "l": 73.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Stone-Mid-Cool™",
    "number": "66",
    "lightness": 54,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Grey Moss™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Furrow™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Mid Azure Green™"
        ]
      }
    ],
    "color": "#D9CFAB",
    "hsl": {
      "h": 47,
      "s": 37.7,
      "l": 76.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Boo™",
    "lightness": 22,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Dorchester Pink - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Perennial Grey™",
          "Welcome - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dorchester Pink®",
          "Pink Slip®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#B16B8D",
    "hsl": {
      "h": 331,
      "s": 31,
      "l": 55.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Stone-Pale-Cool™",
    "number": "65",
    "lightness": 63,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Rolling Fog - Dark™",
          "Silt®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Olive Colour™",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Drummond™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Attic II™"
        ]
      }
    ],
    "color": "#E2D29B",
    "hsl": {
      "h": 46,
      "s": 55,
      "l": 74.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Burton Pink™",
    "lightness": 62,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Maldive™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Julie's Dream™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Conche™",
          "Terra di Sienna™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Bronze Red®"
        ]
      }
    ],
    "color": "#FAD2AE",
    "hsl": {
      "h": 28,
      "s": 88.4,
      "l": 83.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Straw Colour™",
    "lightness": 55,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "White Lead™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Creamerie™",
          "Stone-Pale-Warm™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Buff™",
          "Tan™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Middle Buff™"
        ]
      }
    ],
    "color": "#E8C96E",
    "hsl": {
      "h": 45,
      "s": 72.6,
      "l": 67.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Callaghan™",
    "number": "214",
    "lightness": 12,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dash of Soot™",
          "Mirage II™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Clay™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#6D3D33",
    "hsl": {
      "h": 10,
      "s": 36.2,
      "l": 31.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Sundial™",
    "lightness": 82,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Cricket White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "White Lead - Deep™",
          "Stone-Pale-Cool™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Apple™",
          "Straw Colour™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#FFF6CF",
    "hsl": {
      "h": 49,
      "s": 100,
      "l": 90.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Cape Red™",
    "number": "279",
    "lightness": 12,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "French Grey™",
          "Rubine Ashes™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Furrow™",
          "Pleat™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Thai Sapphire™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#A6393E",
    "hsl": {
      "h": 357,
      "s": 48.9,
      "l": 43.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Celestial White™",
    "number": "262",
    "lightness": 74,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Aquamarine - Mid™",
          "Salix™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine™",
          "Celestial Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Pall Mall™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Tea with Florence™"
        ]
      }
    ],
    "color": "#E2F1EF",
    "hsl": {
      "h": 172,
      "s": 34.9,
      "l": 91.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Tan Tan™",
    "lightness": 26,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "White Lead™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Stone-Pale-Warm™",
          "Tusk™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Mortlake Yellow™",
          "Straw Colour™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Spanish Brown™"
        ]
      }
    ],
    "color": "#BC7C56",
    "hsl": {
      "h": 22,
      "s": 43.2,
      "l": 53.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Chiffon™",
    "lightness": 43,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Lemon Tree™",
          "White Lead - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Citrine™",
          "Jewel Beetle™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Trumpet™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Toad™"
        ]
      }
    ],
    "color": "#C4AF60",
    "hsl": {
      "h": 47,
      "s": 45.9,
      "l": 57.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Terra di Sienna™",
    "lightness": 37,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Hollyhock™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Beauvais Lilac™",
          "Stone-Pale-Warm™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Indian Sand™",
          "Inner Shell II™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Drummond™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Callaghan™"
        ]
      }
    ],
    "color": "#D09D6A",
    "hsl": {
      "h": 30,
      "s": 52,
      "l": 61.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Clapperboard™",
    "lightness": 0,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clockface™",
          "Tusk™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Bronze Green™",
          "Sir Lutyens’ Sage™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#D5C7A5",
    "hsl": {
      "h": 42,
      "s": 36.4,
      "l": 74.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Thatch™",
    "lightness": 64,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Maldive™",
          "Papyrus™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Terra di Sienna™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#E2CEB6",
    "hsl": {
      "h": 33,
      "s": 43.1,
      "l": 80
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Clockface™",
    "number": "81",
    "lightness": 79,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone™",
          "Slaked Lime - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Portland Stone - Dark™",
          "Serpentine™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Grey Moss™"
        ]
      }
    ],
    "color": "#EEEDE8",
    "hsl": {
      "h": 50,
      "s": 15,
      "l": 92.2
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Ticking™",
    "lightness": 66,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Sundial™",
          "White Lead - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Gold™",
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Light Bronze Green™"
        ]
      }
    ],
    "color": "#F3D177",
    "hsl": {
      "h": 44,
      "s": 83.8,
      "l": 71
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Colonial Blue™",
    "lightness": 37,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Bone China Blue - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Ceviche™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Drizzle®",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#9BA7A7",
    "hsl": {
      "h": 180,
      "s": 6.4,
      "l": 63.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Toad™",
    "number": "235",
    "lightness": 8,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Ceviche™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Normandy Grey™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#382F24",
    "hsl": {
      "h": 33,
      "s": 21.7,
      "l": 18
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Cone™",
    "lightness": 51,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "50s Magnolia™",
          "Old Paper™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Indian Sand™",
          "Terra di Sienna™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#D5BB86",
    "hsl": {
      "h": 40,
      "s": 48.5,
      "l": 68
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Tracery™",
    "lightness": 58,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pearl Colour - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Pearl Colour™",
          "Pique™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Sage Green™",
          "Windmill Lane™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#C3CBB6",
    "hsl": {
      "h": 83,
      "s": 16.8,
      "l": 75.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Courtyard™",
    "lightness": 58,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "Slaked Lime - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Normandy Grey™",
          "North Brink Grey™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sage & Onions™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#D3CFC3",
    "hsl": {
      "h": 45,
      "s": 15.4,
      "l": 79.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Tropez Blue™",
    "lightness": 32,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Delicate Blue™",
          "Drizzle®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Brighton™",
          "Stock - Deep™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Green Verditer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Hicks' Blue®"
        ]
      }
    ],
    "color": "#73B7C2",
    "hsl": {
      "h": 188,
      "s": 39.3,
      "l": 60.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Creamerie™",
    "number": "42",
    "lightness": 78,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Aged Ivory™",
          "Stock - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Mortlake Yellow™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#FBF2C9",
    "hsl": {
      "h": 49,
      "s": 86.2,
      "l": 88.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Turquoise Blue™",
    "number": "93",
    "lightness": 38,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Pendula™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hidey Hole™",
          "Ulla™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Canton™",
          "Pleat™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Trumpet™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Mid Azure Green™"
        ]
      }
    ],
    "color": "#6EB399",
    "hsl": {
      "h": 157,
      "s": 31.2,
      "l": 56.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Crinkle™",
    "lightness": 47,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Papyrus™",
          "Sweet Cream™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Toad™"
        ]
      }
    ],
    "color": "#E4D3A9",
    "hsl": {
      "h": 43,
      "s": 52.2,
      "l": 77.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Tusk™",
    "number": "237",
    "lightness": 78,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mortar™",
          "Slaked Lime - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hammock™",
          "Light Peachblossom™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#F1EBE1",
    "hsl": {
      "h": 37,
      "s": 36.4,
      "l": 91.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Cupboard Green®",
    "number": "201",
    "lightness": 65,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Linen Wash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Rolling Fog®",
          "True Taupe™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine - Deep™",
          "Pea Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Ashes of Roses™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Ho Ho Green™"
        ]
      }
    ],
    "color": "#C4DBB8",
    "hsl": {
      "h": 99,
      "s": 32.7,
      "l": 79
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Ulla™",
    "number": "290",
    "lightness": 61,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "North Brink Grey™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pleat™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Cape Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Attic II™"
        ]
      }
    ],
    "color": "#D6DAC6",
    "hsl": {
      "h": 72,
      "s": 21.3,
      "l": 81.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Daffodil™",
    "number": "136",
    "lightness": 51,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "White Lead - Dark™",
          "White Lead - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Middle Buff™",
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Furrow™"
        ]
      }
    ],
    "color": "#DDC46D",
    "hsl": {
      "h": 47,
      "s": 62.2,
      "l": 64.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Weekend™",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Gauze®",
          "Shallows™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Gauze - Mid™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pale Lupin™",
          "Woad™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Scree™"
        ]
      }
    ],
    "color": "#BEC1CA",
    "hsl": {
      "h": 225,
      "s": 10.2,
      "l": 76.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Dart™",
    "lightness": 25,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Limestone™",
          "Mortar™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "China Clay - Dark™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Attic II™"
        ]
      }
    ],
    "color": "#948777",
    "hsl": {
      "h": 33,
      "s": 11.9,
      "l": 52.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Welcome - Dark™",
    "number": "181",
    "lightness": 49,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Welcome - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Arquerite™",
          "Dark Lead Colour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Adventurer™",
          "Pale Lupin™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#BAB0B0",
    "hsl": {
      "h": 0,
      "s": 6.8,
      "l": 71
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Debutant™",
    "lightness": 46,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Chemise™",
          "Dorchester Pink - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Welcome™",
          "Welcome - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hortense™",
          "Pink Slip®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Adventurer™"
        ]
      }
    ],
    "color": "#CDA1B2",
    "hsl": {
      "h": 337,
      "s": 30.6,
      "l": 71.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Welcome - Pale™",
    "number": "179",
    "lightness": 77,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mirage II™",
          "Perennial Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dorchester Pink®",
          "Hortense™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#F1EDEB",
    "hsl": {
      "h": 20,
      "s": 17.6,
      "l": 93.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Dolphin™",
    "number": "246",
    "lightness": 31,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Down™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dash of Soot™",
          "Knightsbridge™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "China Clay - Dark™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#877970",
    "hsl": {
      "h": 23,
      "s": 9.3,
      "l": 48.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Whisper™",
    "number": "5",
    "lightness": 65,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Dorchester Pink - Pale™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Down™",
          "Welcome - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hortense™",
          "Pale Lupin™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Arquerite™"
        ]
      }
    ],
    "color": "#E2DADE",
    "hsl": {
      "h": 330,
      "s": 12.1,
      "l": 87.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Dorchester Pink - Mid™",
    "number": "286",
    "lightness": 72,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dolphin™",
          "Perennial Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Blush™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#EBE5E3",
    "hsl": {
      "h": 15,
      "s": 16.7,
      "l": 90.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "White Lead - Dark™",
    "number": "172",
    "lightness": 66,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Stone-Mid-Cool™",
          "White Lead - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Oak Apple™",
          "Woodbine™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#EADF9D",
    "hsl": {
      "h": 51,
      "s": 64.7,
      "l": 76.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Dorchester Pink - Pale™",
    "number": "285",
    "lightness": 79,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dash of Soot™",
          "Perennial Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Hellebore™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#F8F3F0",
    "hsl": {
      "h": 22,
      "s": 36.4,
      "l": 95.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "White Lead - Mid™",
    "number": "170",
    "lightness": 80,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Stone-Mid-Cool™",
          "Stone-Pale-Cool™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Edith’s Eye™",
          "Sir Lutyens’ Sage™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#F5F1CE",
    "hsl": {
      "h": 54,
      "s": 66.1,
      "l": 88.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Driftwood™",
    "lightness": 53,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Hollyhock™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Julie's Dream™",
          "Roman Plaster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Blush™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#D4BDAC",
    "hsl": {
      "h": 25,
      "s": 31.7,
      "l": 75.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Windjammer™",
    "lightness": 64,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Sundial™",
          "White Lead - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Stargazer™",
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Spanish Brown™"
        ]
      }
    ],
    "color": "#F9CB75",
    "hsl": {
      "h": 39,
      "s": 91.7,
      "l": 71.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Drummond™",
    "number": "16",
    "lightness": 12,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Clay - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Limestone™",
          "Stone-Pale-Cool™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#8F3626",
    "hsl": {
      "h": 9,
      "s": 58,
      "l": 35.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Woodland™",
    "lightness": 23,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Pendula™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Pearl Colour™",
          "Ulla™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine™",
          "Pea Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Green Verditer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Mid Azure Green™"
        ]
      }
    ],
    "color": "#5F8C78",
    "hsl": {
      "h": 153,
      "s": 19.1,
      "l": 46.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Eau-de-Nil™",
    "number": "90",
    "lightness": 53,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Acorn™",
          "Joanna®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Jewel Beetle™",
          "Sir Lutyens’ Sage™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#C8D295",
    "hsl": {
      "h": 70,
      "s": 40.4,
      "l": 70.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Woven™",
    "lightness": 81,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay - Deep™",
          "Portland Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Normandy Grey™",
          "Sir Lutyens’ Sage™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#F8E5D6",
    "hsl": {
      "h": 26,
      "s": 70.8,
      "l": 90.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "id": "edith-s-eye",
    "title": "Edith’s Eye™",
    "number": "301",
    "lightness": 45,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pique™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Furrow™",
          "Silt®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Middle Buff™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Toad™"
        ]
      }
    ],
    "color": "#C5BD7A",
    "hsl": {
      "h": 54,
      "s": 39.3,
      "l": 62.5
    },
    "categories": [
      "Archive"
    ],
    "aliases": [
      "Edith's Eye"
    ]
  },
  {
    "title": "Felt™",
    "number": "145",
    "lightness": 14,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Dark™",
          "China Clay - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dorchester Pink®",
          "Light Peachblossom™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#524138",
    "hsl": {
      "h": 21,
      "s": 18.8,
      "l": 27.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Fray™",
    "lightness": 11,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Brighton™",
          "Sky Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#3A6472",
    "hsl": {
      "h": 195,
      "s": 32.6,
      "l": 33.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Furrow™",
    "number": "241",
    "lightness": 14,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Tusk™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mortar™",
          "True Taupe™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Pale Lime™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#605241",
    "hsl": {
      "h": 33,
      "s": 19.3,
      "l": 31.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Gentle Sky™",
    "number": "102",
    "lightness": 71,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mid Lead Colour™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Celestial Blue™",
          "Sky Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Air Force Blue™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#C6E1E0",
    "hsl": {
      "h": 178,
      "s": 31,
      "l": 82.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Gourdy™",
    "lightness": 40,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Stone-Pale-Cool™",
          "White Lead - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Carys™",
          "Middle Buff™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Light Bronze Green™"
        ]
      }
    ],
    "color": "#DA9E46",
    "hsl": {
      "h": 36,
      "s": 66.7,
      "l": 56.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Grapevine™",
    "lightness": 31,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Rolling Fog - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hammock™",
          "Rolling Fog - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Burton Pink™",
          "Shrimp Pink™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Atomic Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#B39074",
    "hsl": {
      "h": 27,
      "s": 29.3,
      "l": 57.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Grey Teal™",
    "number": "226",
    "lightness": 29,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Shallows™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bone China Blue - Deep™",
          "Mono™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pearl Colour™",
          "Pink Slip®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#898983",
    "hsl": {
      "h": 60,
      "s": 2.5,
      "l": 52.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Ho Ho Green™",
    "number": "305",
    "lightness": 15,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Portland Stone - Pale™",
          "Tusk™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Slaked Lime - Dark™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine - Deep™",
          "Celestial Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Green Verditer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#5C6F63",
    "hsl": {
      "h": 142,
      "s": 9.4,
      "l": 39.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Hortense™",
    "number": "266",
    "lightness": 55,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Down™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dolphin™",
          "Mid Lead Colour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Adventurer™",
          "Silt®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Scree™"
        ]
      }
    ],
    "color": "#D4CBD5",
    "hsl": {
      "h": 294,
      "s": 10.6,
      "l": 81.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Inner Shell™",
    "lightness": 72,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Deep™",
          "Rolling Fog®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Button™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#F6D6C7",
    "hsl": {
      "h": 19,
      "s": 72.3,
      "l": 87.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Ivory™",
    "number": "62",
    "lightness": 67,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Creamerie™",
          "Stock - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Light Buff™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#F6DC96",
    "hsl": {
      "h": 44,
      "s": 84.2,
      "l": 77.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Jean™",
    "lightness": 30,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Drizzle®",
          "Pale Wedgwood™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Deep Space Blue™",
          "Woad™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mambo™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#8492D4",
    "hsl": {
      "h": 230,
      "s": 48.2,
      "l": 67.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "King Cup™",
    "lightness": 61,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "50s Magnolia™",
          "Papyrus™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Citrine™",
          "Stargazer™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#F5C579",
    "hsl": {
      "h": 37,
      "s": 86.1,
      "l": 71.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Lagoon™",
    "lightness": 35,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Gauze®",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Gauze - Deep™",
          "Gauze - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pale Lupin™",
          "Smalt™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#78A3CD",
    "hsl": {
      "h": 210,
      "s": 45.9,
      "l": 63.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Lead White™",
    "number": "57",
    "lightness": 78,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mortar™",
          "Portland Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Sir Lutyens’ Sage™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sage & Onions™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#F4F0E4",
    "hsl": {
      "h": 45,
      "s": 42.1,
      "l": 92.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Lemon Tree™",
    "lightness": 64,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Furrow™",
          "Grey Moss™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Citrine™",
          "Olive Colour™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#ECDD87",
    "hsl": {
      "h": 51,
      "s": 72.7,
      "l": 72.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Light Buff™",
    "lightness": 43,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Old Paper II™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Aged Ivory™",
          "Stone-Pale-Warm™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Cream Colour™",
          "Middle Buff™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#CEA95B",
    "hsl": {
      "h": 41,
      "s": 54,
      "l": 58.2
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Light Wicker™",
    "lightness": 58,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "White Lead™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "50s Magnolia™",
          "Old Paper™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Buff™",
          "Pantry Cream™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#ECC18A",
    "hsl": {
      "h": 34,
      "s": 72.1,
      "l": 73.3
    },
    "categories": [
      "Archive"
    ],
    "aliases": [
      "Dooly™"
    ]
  },
  {
    "title": "Limestone™",
    "number": "238",
    "lightness": 61,
    "connections": [],
    "color": "#DDD4C3",
    "hsl": {
      "h": 39,
      "s": 27.7,
      "l": 81.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Lopez™",
    "lightness": 62,
    "connections": [],
    "color": "#FFC636",
    "hsl": {
      "h": 43,
      "s": 100,
      "l": 60.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Mandarin™",
    "lightness": 31,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "White Lead™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Orangerie™",
          "Stone-Pale-Warm™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Burton Pink™",
          "Jersey Cream™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#E77843",
    "hsl": {
      "h": 19,
      "s": 77.4,
      "l": 58.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Mid Brunswick Green™",
    "lightness": 11,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Linen Wash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Limestone™",
          "Portland Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pea Green™",
          "Tabernacle™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Phthalo Green™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#4B654A",
    "hsl": {
      "h": 118,
      "s": 15.4,
      "l": 34.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Milano™",
    "lightness": 79,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bone China Blue™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Grey Stone™",
          "James™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#DFE6F0",
    "hsl": {
      "h": 215,
      "s": 36.2,
      "l": 90.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Mink™",
    "lightness": 22,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Rolling Fog - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Rubine Ashes™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Hellebore™",
          "Light Peachblossom™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#927B73",
    "hsl": {
      "h": 15,
      "s": 12.4,
      "l": 51.2
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Mirage II™",
    "number": "4",
    "lightness": 57,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Down™",
          "Welcome - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dolphin™",
          "Perennial Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Mischief™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Ashes of Roses™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Knightsbridge™"
        ]
      }
    ],
    "color": "#D1C0B2",
    "hsl": {
      "h": 27,
      "s": 25.2,
      "l": 75.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Mono™",
    "number": "218",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dark Lead Colour™",
          "Grey Teal™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Arquerite™",
          "Juniper Ash®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mambo™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#C3C2B9",
    "hsl": {
      "h": 54,
      "s": 7.7,
      "l": 74.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Morning™",
    "lightness": 70,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bone China Blue - Deep™",
          "Mid Lead Colour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Grey Stone™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Blue Verditer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Hicks' Blue®"
        ]
      }
    ],
    "color": "#D1DCDE",
    "hsl": {
      "h": 189,
      "s": 16.5,
      "l": 84.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Mortlake Yellow™",
    "number": "265",
    "lightness": 47,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "True Taupe™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Sage Green™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Drummond™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Toad™"
        ]
      }
    ],
    "color": "#DEB879",
    "hsl": {
      "h": 37,
      "s": 60.5,
      "l": 67.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Narcissus™",
    "lightness": 76,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Normandy Grey™",
          "Tracery II™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Edith’s Eye™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sage & Onions™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jewel Beetle™"
        ]
      }
    ],
    "color": "#FFF298",
    "hsl": {
      "h": 52,
      "s": 100,
      "l": 79.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Old Paper™",
    "lightness": 76,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hammock™",
          "Mushroom™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Peachblossom™",
          "Roman Plaster™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Adventurer™"
        ]
      }
    ],
    "color": "#F2EECB",
    "hsl": {
      "h": 54,
      "s": 60,
      "l": 87.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Old School Blue™",
    "number": "259",
    "lightness": 25,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Delicate Blue™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Brighton™",
          "Sky Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Phthalo Green™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#4999AE",
    "hsl": {
      "h": 192,
      "s": 40.9,
      "l": 48.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Orangerie™",
    "lightness": 78,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mushroom™",
          "Roman Plaster™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Cream Colour™",
          "Stone-Mid-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Callaghan™"
        ]
      }
    ],
    "color": "#F8EDDB",
    "hsl": {
      "h": 37,
      "s": 67.4,
      "l": 91.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Palazzo™",
    "lightness": 60,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Boringdon Green™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Citrine™",
          "Sir Lutyens’ Sage™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sage & Onions™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jewel Beetle™"
        ]
      }
    ],
    "color": "#FFC12E",
    "hsl": {
      "h": 42,
      "s": 100,
      "l": 59
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Pall Mall™",
    "number": "309",
    "lightness": 44,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Grey Teal™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Arquerite™",
          "Pleat™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Adventurer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Trumpet™"
        ]
      }
    ],
    "color": "#96C2B8",
    "hsl": {
      "h": 166,
      "s": 26.5,
      "l": 67.5
    },
    "categories": [
      "Archive",
      "1930s"
    ]
  },
  {
    "title": "Pantry Cream™",
    "lightness": 78,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay - Deep™",
          "Clay - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Light Buff™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Spanish Brown™"
        ]
      }
    ],
    "color": "#FDE781",
    "hsl": {
      "h": 49,
      "s": 96.9,
      "l": 74.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Paradise™",
    "lightness": 94,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Stone-Mid-Cool™",
          "Stone-Dark-Cool™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Oak Apple™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Olive Colour™"
        ]
      }
    ],
    "color": "#FFF9D6",
    "hsl": {
      "h": 51,
      "s": 100,
      "l": 92
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Peaceful™",
    "lightness": 42,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pearl Colour - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Pearl Colour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Canton™",
          "Pleat™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Cape Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Harley Green™"
        ]
      }
    ],
    "color": "#A8AFA4",
    "hsl": {
      "h": 98,
      "s": 6.4,
      "l": 66.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Pencil™",
    "lightness": 64,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Gauze®",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Grey Teal™",
          "Mid Lead Colour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Grey Stone™",
          "Juniper Ash®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Deep Space Blue™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#CED1D8",
    "hsl": {
      "h": 222,
      "s": 11.4,
      "l": 82.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Penelope™",
    "lightness": 64,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Perennial Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Carmine™",
          "Hellebore™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#EFCEDF",
    "hsl": {
      "h": 329,
      "s": 50.8,
      "l": 87.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Pillar™",
    "lightness": 61,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Batsman™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone - Dark™",
          "Tusk™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Silt®",
          "True Taupe™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Attic II™"
        ]
      }
    ],
    "color": "#D6CCB9",
    "hsl": {
      "h": 39,
      "s": 26.1,
      "l": 78.2
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Pique™",
    "number": "299",
    "lightness": 78,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Lead Colour™",
          "Pearl Colour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Jewel Beetle™",
          "Pea Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Drummond™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#F0F6E4",
    "hsl": {
      "h": 80,
      "s": 50,
      "l": 92.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Polar Blue™",
    "lightness": 37,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Echo™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mid Lead Colour™",
          "Mono™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Brighton™",
          "Sky Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Trumpet™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#72B0BB",
    "hsl": {
      "h": 189,
      "s": 34.9,
      "l": 59
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Portland Stone - Deep™",
    "number": "156",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Portland Stone - Pale™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Grey Moss™",
          "Putti™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Drummond™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#BAB492",
    "hsl": {
      "h": 51,
      "s": 22.5,
      "l": 65.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Purpleheart™",
    "number": "188",
    "lightness": 8,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Rusling™",
          "Dorchester Pink - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dash of Soot™",
          "Mono™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Arquerite™",
          "Hortense™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#432F5A",
    "hsl": {
      "h": 268,
      "s": 31.4,
      "l": 26.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Quirky™",
    "lightness": 30,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Aquamarine - Mid™",
          "Salix™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Canton™",
          "Pleat™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Trumpet™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Harley Green™"
        ]
      }
    ],
    "color": "#629FA0",
    "hsl": {
      "h": 181,
      "s": 24.6,
      "l": 50.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Rattan™",
    "lightness": 53,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "White Lead™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "50s Magnolia™",
          "Papyrus™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Indian Sand™",
          "Jersey Cream™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Spanish Brown™"
        ]
      }
    ],
    "color": "#F4B379",
    "hsl": {
      "h": 28,
      "s": 84.8,
      "l": 71.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Regency Blue™",
    "number": "253",
    "lightness": 48,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Delicate Blue™",
          "Drizzle®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Route One™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sage & Onions™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#92CADE",
    "hsl": {
      "h": 196,
      "s": 53.5,
      "l": 72.2
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Revere™",
    "lightness": 43,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Stone-Pale-Cool™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Citrine™",
          "Sir Lutyens’ Sage™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Trumpet™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Olive Colour™"
        ]
      }
    ],
    "color": "#C0AF67",
    "hsl": {
      "h": 49,
      "s": 41.4,
      "l": 57.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Route One™",
    "number": "254",
    "lightness": 29,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Drizzle®",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Moon Shadow™",
          "Sky Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Phthalo Green™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#50A2C2",
    "hsl": {
      "h": 197,
      "s": 48.3,
      "l": 53.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Sage & Onions™",
    "number": "288",
    "lightness": 19,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay - Deep™",
          "Limestone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Gold™",
          "Woad™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Toad™"
        ]
      }
    ],
    "color": "#638538",
    "hsl": {
      "h": 86,
      "s": 40.7,
      "l": 37.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Secret Shell™",
    "lightness": 76,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Chemise™",
          "Hortense™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Confetti™",
          "Hellebore™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#FFE4E0",
    "hsl": {
      "h": 8,
      "s": 100,
      "l": 93.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Shift™",
    "lightness": 7,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mortar™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Heat™",
          "Middle Buff™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Drummond™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#4B4B49",
    "hsl": {
      "h": 60,
      "s": 1.4,
      "l": 29
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Simplicity™",
    "lightness": 61,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Hollyhock™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Julie's Dream™",
          "Old Paper™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#DCC9BB",
    "hsl": {
      "h": 25,
      "s": 32,
      "l": 79.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Skinny™",
    "lightness": 65,
    "connections": [],
    "color": "#DAD1D0",
    "hsl": {
      "h": 6,
      "s": 11.9,
      "l": 83.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "50s Magnolia™",
    "number": "28",
    "lightness": 74,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Mushroom™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Mortlake Yellow™",
          "Stone-Mid-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Furrow™"
        ]
      }
    ],
    "color": "#F4EBDC",
    "hsl": {
      "h": 37,
      "s": 52.2,
      "l": 91
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Spanish Brown™",
    "number": "32",
    "lightness": "7.42",
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mortar™",
          "Rolling Fog®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Mortlake Yellow™",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#3F2B1F",
    "hsl": {
      "h": 23,
      "s": 34,
      "l": 18.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Acre™",
    "number": "76",
    "lightness": 63,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone - Dark™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Rolling Fog - Dark™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Livid™"
        ]
      }
    ],
    "color": "#E6E4CD",
    "hsl": {
      "h": 55,
      "s": 33.3,
      "l": 85.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Spearmint™",
    "number": "202",
    "lightness": 47,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hidey Hole™",
          "Salix™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Livid™",
          "Tea with Florence™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Harley Green™"
        ]
      }
    ],
    "color": "#A3C89B",
    "hsl": {
      "h": 109,
      "s": 29,
      "l": 69.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Al Fresco™",
    "lightness": 35,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Down™",
          "Rolling Fog - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Carmine™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Adventurer™"
        ]
      }
    ],
    "color": "#A79CA4",
    "hsl": {
      "h": 316,
      "s": 5.9,
      "l": 63.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Starling's Egg™",
    "number": "97",
    "lightness": 83,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Aquamarine - Mid™",
          "Salix™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pea Green™",
          "Tabernacle™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Green Verditer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Goblin™"
        ]
      }
    ],
    "color": "#EDF4ED",
    "hsl": {
      "h": 120,
      "s": 24.1,
      "l": 94.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Ballroom Cream™",
    "lightness": 82,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Indian Sand™",
          "Stone-Pale-Warm™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Cream Colour™",
          "Straw Colour™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Bronze Red®"
        ]
      }
    ],
    "color": "#FAF5B5",
    "hsl": {
      "h": 56,
      "s": 87.3,
      "l": 84.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Stock - Mid™",
    "number": "173",
    "lightness": 74,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "True Taupe™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Mortlake Yellow™",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Toad™"
        ]
      }
    ],
    "color": "#EFE4C9",
    "hsl": {
      "h": 43,
      "s": 54.3,
      "l": 86.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Basque™",
    "lightness": 13,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mortar™",
          "Mushroom™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Mortlake Yellow™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#8E555C",
    "hsl": {
      "h": 353,
      "s": 25.1,
      "l": 44.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Stole™",
    "lightness": 10,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Rolling Fog®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Drummond™",
          "Mortlake Yellow™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Atomic Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#625954",
    "hsl": {
      "h": 21,
      "s": 7.7,
      "l": 35.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Beach Hut™",
    "lightness": 34,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Echo™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pea Green™",
          "Sky Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sage & Onions™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#53A8D1",
    "hsl": {
      "h": 200,
      "s": 57.8,
      "l": 57.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Stone-Dark-Warm™",
    "number": "36",
    "lightness": 31,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hammock™",
          "Limestone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Clay™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#A28555",
    "hsl": {
      "h": 37,
      "s": 31.2,
      "l": 48.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Bone China Blue - Deep™",
    "number": "184",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Gauze®",
          "Shallows™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Drizzle®",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Grey Stone™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dock Blue™"
        ]
      }
    ],
    "color": "#B0B5B2",
    "hsl": {
      "h": 144,
      "s": 3.3,
      "l": 70
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Stone-Mid-Warm™",
    "number": "35",
    "lightness": 51,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay - Mid™",
          "Stone-Pale-Warm™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Middle Buff™",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Light Bronze Green™"
        ]
      }
    ],
    "color": "#DEC58F",
    "hsl": {
      "h": 41,
      "s": 54.5,
      "l": 71.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Brilliant Green™",
    "lightness": 15,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Acorn™",
          "Ulla™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Apple™",
          "Light Gold™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Phthalo Green™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#477841",
    "hsl": {
      "h": 113,
      "s": 29.7,
      "l": 36.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Strappy™",
    "lightness": 16,
    "connections": [],
    "color": "#796A81",
    "hsl": {
      "h": 279,
      "s": 9.8,
      "l": 46.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Button™",
    "number": "27",
    "lightness": 41,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Deep™",
          "Joanna®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Callaghan™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Angie™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Spanish Brown™"
        ]
      }
    ],
    "color": "#CCB297",
    "hsl": {
      "h": 31,
      "s": 34.2,
      "l": 69.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Summer Cloud™",
    "lightness": 44,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Gauze®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Grey Teal™",
          "Mid Lead Colour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Juniper Ash®",
          "Woad™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#B3B1B7",
    "hsl": {
      "h": 260,
      "s": 4,
      "l": 70.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Camisole™",
    "lightness": 93,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Limestone™",
          "Slaked Lime - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Mortlake Yellow™",
          "Oak Apple™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Furrow™"
        ]
      }
    ],
    "color": "#F6F8EB",
    "hsl": {
      "h": 69,
      "s": 48.1,
      "l": 94.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Sweet Cream™",
    "lightness": 79,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay - Deep™",
          "Stock - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Cream Colour™",
          "Terra di Sienna™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Spanish Brown™"
        ]
      }
    ],
    "color": "#F3E8B0",
    "hsl": {
      "h": 50,
      "s": 73.6,
      "l": 82.2
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Carnival™",
    "lightness": 28,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bone China Blue - Mid™",
          "French Grey - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Mazarine™",
          "Woad™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#7095A9",
    "hsl": {
      "h": 201,
      "s": 24.9,
      "l": 55.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Tan™",
    "lightness": 20,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "White Lead™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay - Deep™",
          "Joanna®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Mortlake Yellow™",
          "Stone-Pale-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#8B5C26",
    "hsl": {
      "h": 32,
      "s": 57.1,
      "l": 34.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Chamois™",
    "number": "132",
    "lightness": 54,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "White Lead™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Stock - Mid™",
          "Stock - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Mortlake Yellow™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Furrow™"
        ]
      }
    ],
    "color": "#E2D39A",
    "hsl": {
      "h": 48,
      "s": 55.4,
      "l": 74.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Tash™",
    "lightness": 71,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Maldive™",
          "Sweet Cream™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Buff™",
          "Middle Buff™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Spanish Brown™"
        ]
      }
    ],
    "color": "#FDD68C",
    "hsl": {
      "h": 39,
      "s": 96.6,
      "l": 77.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Chimney Brick™",
    "number": "247",
    "lightness": 7,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Down™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dash of Soot™",
          "Dolphin™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Stone-Dark-Warm™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#3A2A23",
    "hsl": {
      "h": 18,
      "s": 24.7,
      "l": 18.2
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Terrace™",
    "lightness": 28,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Mirror™",
          "Pearl Colour - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Pearl Colour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Livid™",
          "Salix™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Tabernacle™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Goblin™"
        ]
      }
    ],
    "color": "#859B8D",
    "hsl": {
      "h": 142,
      "s": 9.9,
      "l": 56.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Clay - Deep™",
    "number": "154",
    "lightness": 61,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Silt®",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Tuscan Red™",
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Furrow™"
        ]
      }
    ],
    "color": "#DFD3B4",
    "hsl": {
      "h": 43,
      "s": 40.2,
      "l": 79
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Theatrical™",
    "lightness": 37,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Beauvais Lilac™",
          "Limestone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Drummond™",
          "Heat™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Adventurer™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#F08861",
    "hsl": {
      "h": 16,
      "s": 82.7,
      "l": 66.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Clockwise™",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Gauze®",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Arquerite™",
          "Mid Lead Colour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Juniper Ash®",
          "Pale Lupin™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mambo™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#9FA2CF",
    "hsl": {
      "h": 236,
      "s": 33.3,
      "l": 71.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Titley Green™",
    "lightness": 24,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pearl Colour - Pale™",
          "Pendula™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Inox™",
          "Ulla™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Acorn™",
          "Salix™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#808879",
    "hsl": {
      "h": 92,
      "s": 5.9,
      "l": 50.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Conche™",
    "lightness": 52,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Maldive™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Julie's Dream™",
          "Inner Shell II™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Button™",
          "Heat™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Adventurer™"
        ]
      }
    ],
    "color": "#F7B578",
    "hsl": {
      "h": 29,
      "s": 88.8,
      "l": 72
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Totally Tom™",
    "lightness": 13,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mortar™",
          "Mushroom™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Gold™",
          "Mortlake Yellow™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#AB423F",
    "hsl": {
      "h": 2,
      "s": 46.2,
      "l": 45.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Cork™",
    "lightness": 6,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dash of Soot™",
          "Rubine Ashes™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Terra di Sienna™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#292017",
    "hsl": {
      "h": 30,
      "s": 28.1,
      "l": 12.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Tranquil™",
    "lightness": 88,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mirage II™",
          "Rusling™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Carmine™",
          "Hellebore™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Leather™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Adventurer™"
        ]
      }
    ],
    "color": "#FFE9DB",
    "hsl": {
      "h": 23,
      "s": 100,
      "l": 92.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Cream Colour™",
    "lightness": 63,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Old Paper II™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Indian Sand™",
          "Orangerie™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Buff™",
          "Middle Buff™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#E6D09F",
    "hsl": {
      "h": 41,
      "s": 58.7,
      "l": 76.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "True Taupe™",
    "number": "240",
    "lightness": 33,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Tusk™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Furrow™",
          "Limestone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Aquamarine - Mid™",
          "Livid™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#B5A791",
    "hsl": {
      "h": 37,
      "s": 19.6,
      "l": 63.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Cricket White™",
    "lightness": 85,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "White Lead - Dark™",
          "White Lead - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Edith’s Eye™",
          "Sunlight™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Olive Colour™"
        ]
      }
    ],
    "color": "#F3FFCF",
    "hsl": {
      "h": 75,
      "s": 100,
      "l": 90.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Tuscany™",
    "lightness": 45,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Deep™",
          "Hammock™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#D8B197",
    "hsl": {
      "h": 24,
      "s": 45.5,
      "l": 72
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Crush™",
    "lightness": 44,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "White Lead™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Limestone™",
          "Stock - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ambleside™",
          "Sir Lutyens’ Sage™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Jewel Beetle™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#F69C50",
    "hsl": {
      "h": 27,
      "s": 90.2,
      "l": 63.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Twist™",
    "lightness": 44,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Celestial White™",
          "Mirror™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Harley Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Slipper™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Tea with Florence™"
        ]
      }
    ],
    "color": "#8FB9B7",
    "hsl": {
      "h": 177,
      "s": 23.1,
      "l": 64.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Custard™",
    "number": "133",
    "lightness": 75,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Oak Apple™",
          "White Lead - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Edith’s Eye™",
          "Pale Lime™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#F8E8A6",
    "hsl": {
      "h": 48,
      "s": 85.4,
      "l": 81.2
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Vincent™",
    "lightness": 44,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Gauze®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Ceviche™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bone China Blue - Mid™",
          "Lead Colour™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Air Force Blue™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Scree™"
        ]
      }
    ],
    "color": "#BAB8A3",
    "hsl": {
      "h": 55,
      "s": 14.3,
      "l": 68.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Dark Bronze Green™",
    "lightness": 7,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pique™",
          "Stock®"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "Pearl Colour™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boringdon Green™",
          "Windmill Lane™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Jack Black™"
        ]
      }
    ],
    "color": "#2A341C",
    "hsl": {
      "h": 85,
      "s": 30,
      "l": 15.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Welcome™",
    "number": "109",
    "lightness": 68,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Welcome - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Perennial Grey™",
          "Welcome - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Lead Colour™",
          "Light Peachblossom™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Dark Lead Colour™"
        ]
      }
    ],
    "color": "#E3DCD8",
    "hsl": {
      "h": 22,
      "s": 16.4,
      "l": 86.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Daylight™",
    "lightness": 84,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Pitcairn™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Light Buff™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Spanish Brown™"
        ]
      }
    ],
    "color": "#FFE9C5",
    "hsl": {
      "h": 37,
      "s": 100,
      "l": 88.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Welcome - Deep™",
    "number": "180",
    "lightness": 61,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Arquerite™",
          "Perennial Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Adventurer™",
          "Pale Lupin™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#CDC6C3",
    "hsl": {
      "h": 18,
      "s": 9.1,
      "l": 78.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Desert Wind™",
    "lightness": 25,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Mid™",
          "Mirage II™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Baked Cherry®",
          "Thai Sapphire™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Theatre Red™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#CA7065",
    "hsl": {
      "h": 7,
      "s": 48.8,
      "l": 59.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Whimsey™",
    "lightness": 46,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Shallows™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Pleat™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#A7BBA2",
    "hsl": {
      "h": 108,
      "s": 15.5,
      "l": 68.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Whistle™",
    "lightness": 50,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Hollyhock™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Julie's Dream™",
          "Rolling Fog - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Tuscan Red™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Adventurer™"
        ]
      }
    ],
    "color": "#D9B596",
    "hsl": {
      "h": 28,
      "s": 46.9,
      "l": 72
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Dorchester Pink - Deep™",
    "number": "287",
    "lightness": 64,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Down™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Arquerite™",
          "Dolphin™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Blush™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#E4DBDA",
    "hsl": {
      "h": 6,
      "s": 15.6,
      "l": 87.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "White Lead - Deep™",
    "number": "171",
    "lightness": 75,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Stone-Mid-Cool™",
          "Stone-Pale-Cool™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Edith’s Eye™",
          "Sir Lutyens’ Sage™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Invisible Green™"
        ]
      }
    ],
    "color": "#F0E8B9",
    "hsl": {
      "h": 51,
      "s": 64.7,
      "l": 83.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Dover White™",
    "lightness": 84,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay - Deep™",
          "Stock - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Burton Pink™",
          "Straw Colour™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#FCF9CE",
    "hsl": {
      "h": 56,
      "s": 88.5,
      "l": 89.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Wicken™",
    "lightness": 68,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Stone-Dark-Cool™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Light Bronze Green™"
        ]
      }
    ],
    "color": "#E4D5C1",
    "hsl": {
      "h": 34,
      "s": 39.3,
      "l": 82.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Drizzle®",
    "number": "217",
    "lightness": 68,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mid Lead Colour™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Bone China Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Royal Navy™"
        ]
      }
    ],
    "color": "#D7E7E7",
    "hsl": {
      "h": 180,
      "s": 25,
      "l": 87.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Woodbine™",
    "number": "134",
    "lightness": 65,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Oak Apple™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Olive Colour™",
          "Sage Green™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Furrow™"
        ]
      }
    ],
    "color": "#E4D292",
    "hsl": {
      "h": 47,
      "s": 60.3,
      "l": 73.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Dune™",
    "lightness": 64,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "White Lead™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Dover White™",
          "Papyrus™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Buff™",
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Middle Brown™"
        ]
      }
    ],
    "color": "#F4CD75",
    "hsl": {
      "h": 42,
      "s": 85.2,
      "l": 70.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Wormwood™",
    "number": "300",
    "lightness": 65,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Flint™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Rolling Fog - Dark™",
          "Stone-Dark-Warm™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Citrine™",
          "Edith’s Eye™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#DEE3B1",
    "hsl": {
      "h": 66,
      "s": 47.2,
      "l": 79.2
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Echo™",
    "number": "98",
    "lightness": 77,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Urbane Grey™",
          "Gauze - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Air Force Blue™",
          "Pale Lupin™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Lamp Black™"
        ]
      }
    ],
    "color": "#E8F1EE",
    "hsl": {
      "h": 160,
      "s": 24.3,
      "l": 92.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Far Atoll™",
    "lightness": 54,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Welcome - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Welcome™",
          "Whisper™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Arquerite™",
          "Pale Lupin™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mischief™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Thai Sapphire™"
        ]
      }
    ],
    "color": "#CBBECA",
    "hsl": {
      "h": 305,
      "s": 11.1,
      "l": 77.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Firefly™",
    "lightness": 16,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Fescue™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bronze Red®",
          "Stock - Dark™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#AF2426",
    "hsl": {
      "h": 359,
      "s": 65.9,
      "l": 41.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "French Stone™",
    "lightness": 75,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Linen Wash™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Stone-Dark-Cool™",
          "Stone-Mid-Cool™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bath Stone®",
          "Garden™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Citrine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Light Bronze Green™"
        ]
      }
    ],
    "color": "#FCEABC",
    "hsl": {
      "h": 43,
      "s": 91.4,
      "l": 86.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Gauze - Deep™",
    "number": "165",
    "lightness": 64,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Mid Lead Colour™",
          "Urbane Grey™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Grey Stone™",
          "James™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mambo™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Basalt®"
        ]
      }
    ],
    "color": "#D2D6D9",
    "hsl": {
      "h": 206,
      "s": 8.4,
      "l": 83.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Goddess™",
    "lightness": 62,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pearl Colour - Pale™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hidey Hole™",
          "Windmill Lane™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ambleside™",
          "Pleat™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Canton™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Three Farm Green™"
        ]
      }
    ],
    "color": "#B9D5BE",
    "hsl": {
      "h": 131,
      "s": 25,
      "l": 78
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Grace™",
    "lightness": 81,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Olive Oil™",
          "Ulla™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Apple™",
          "Horizon™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Lemon Mivvi™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Olive Colour™"
        ]
      }
    ],
    "color": "#F4F5E0",
    "hsl": {
      "h": 63,
      "s": 51.2,
      "l": 92
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Grey Moss™",
    "number": "234",
    "lightness": 20,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Fescue™",
          "Wood Ash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Cool Arbour™",
          "Serpentine™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Bone China Blue™",
          "Pearl Colour - Dark™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Hellebore™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#645D51",
    "hsl": {
      "h": 38,
      "s": 10.5,
      "l": 35.5
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Harmony™",
    "lightness": 67,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Down™",
          "Rolling Fog - Mid™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Blush™",
          "Hellebore™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Adventurer™"
        ]
      }
    ],
    "color": "#E2D3CC",
    "hsl": {
      "h": 19,
      "s": 27.5,
      "l": 84.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Horizon™",
    "lightness": 49,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Linen Wash™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Ceviche™",
          "Tusk™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Grey Moss™",
          "Sir Lutyens’ Sage™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sage & Onions™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#C6C690",
    "hsl": {
      "h": 60,
      "s": 32.1,
      "l": 67.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Indian Sand™",
    "number": "54",
    "lightness": 72,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Bath Stone®",
          "Chamois™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Buff™",
          "Light Gold™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#FFE5AA",
    "hsl": {
      "h": 42,
      "s": 100,
      "l": 83.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Inner Shell II™",
    "lightness": 72,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "China Clay™",
          "Rusling™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "China Clay - Dark™",
          "Rolling Fog®"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Ashes of Roses™",
          "Hellebore™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Carmine™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Córdoba™"
        ]
      }
    ],
    "color": "#F7E6D6",
    "hsl": {
      "h": 29,
      "s": 67.3,
      "l": 90.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Jack-In-A-Box™",
    "lightness": 42,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Hidey Hole™",
          "Inox™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Canton™",
          "Marine Blue™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Thai Sapphire™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Goblin™"
        ]
      }
    ],
    "color": "#5ABFAD",
    "hsl": {
      "h": 169,
      "s": 44.1,
      "l": 55.1
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Jersey Cream™",
    "lightness": 66,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Whitening™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Creamerie™",
          "Papyrus™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Light Buff™",
          "Mortlake Yellow™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Middle Buff™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Middle Brown™"
        ]
      }
    ],
    "color": "#F2DD8E",
    "hsl": {
      "h": 47,
      "s": 79.4,
      "l": 75.3
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Knightsbridge™",
    "number": "215",
    "lightness": 16,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Rolling Fog - Pale™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Perennial Grey™",
          "Welcome - Deep™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Dorchester Pink®",
          "Hellebore™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chocolate Colour™"
        ]
      }
    ],
    "color": "#65564E",
    "hsl": {
      "h": 21,
      "s": 12.8,
      "l": 35.1
    },
    "categories": [
      "Archive",
      "1960s"
    ],
    "aliases": [
      "Nightsbridge"
    ]
  },
  {
    "title": "Lawnmower Green™",
    "lightness": 11,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Pendula™",
          "Pique™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Fescue™",
          "Ulla™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Pea Green™",
          "Yellow-Pink™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Mister David™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Obsidian Green™"
        ]
      }
    ],
    "color": "#28471D",
    "hsl": {
      "h": 104,
      "s": 42,
      "l": 19.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Lemon Mivvi™",
    "lightness": 73,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Serpentine™",
          "Tracery II™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Citrine™",
          "Pale Lime™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sage & Onions™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Olive Colour™"
        ]
      }
    ],
    "color": "#FFF371",
    "hsl": {
      "h": 55,
      "s": 100,
      "l": 72.2
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Light™",
    "lightness": 76,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Clay™",
          "Stock - Dark™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Stone-Dark-Warm™",
          "Terra di Sienna™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Marigold™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Chimney Brick™"
        ]
      }
    ],
    "color": "#F1E0C5",
    "hsl": {
      "h": 37,
      "s": 61.1,
      "l": 85.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Light Brunswick Green™",
    "lightness": 23,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Limestone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Edith’s Eye™",
          "Pale Lime™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Trumpet™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Puck™"
        ]
      }
    ],
    "color": "#728D58",
    "hsl": {
      "h": 91,
      "s": 23.1,
      "l": 44.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Light Challinor™",
    "lightness": 63,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Flint™",
          "White Lead™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Portland Stone - Dark™",
          "Tusk™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Boxington®",
          "Sir Lutyens’ Sage™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Sage & Onions™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Olive Colour™"
        ]
      }
    ],
    "color": "#D5CBAD",
    "hsl": {
      "h": 45,
      "s": 32.3,
      "l": 75.7
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Limelight™",
    "lightness": 85,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Limestone™",
          "Portland Stone™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Apple™",
          "Boxington®"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Drummond™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Pompeian Ash™"
        ]
      }
    ],
    "color": "#F4EEDA",
    "hsl": {
      "h": 46,
      "s": 54.2,
      "l": 90.6
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Linnet™",
    "number": "89",
    "lightness": 80,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "First Light™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Apple™",
          "Olive Oil™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Edith’s Eye™",
          "Pale Lime™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Trumpet™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Olive Colour™"
        ]
      }
    ],
    "color": "#F6F2D7",
    "hsl": {
      "h": 52,
      "s": 63.3,
      "l": 90.4
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Maldive™",
    "lightness": 80,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Loft White™",
          "Slaked Lime™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Beauvais Lilac™",
          "Stone-Pale-Warm™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Burton Pink™",
          "Terra di Sienna™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Middle Brown™"
        ]
      }
    ],
    "color": "#FFF3E5",
    "hsl": {
      "h": 32,
      "s": 100,
      "l": 94.9
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Marquee™",
    "lightness": 62,
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Clay - Pale™",
          "Hollyhock™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Down™",
          "Tusk™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Attic II™",
          "Blush™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Orange Aurora™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Roman Plaster™"
        ]
      }
    ],
    "color": "#D6CEC1",
    "hsl": {
      "h": 37,
      "s": 20.4,
      "l": 79.8
    },
    "categories": [
      "Archive"
    ]
  },
  {
    "title": "Re:mix Castell Pink™",
    "number": "314",
    "connections": [
      {
        "title": "Related Whites",
        "colors": [
          "Hollyhock™",
          "Ferdinand™"
        ]
      },
      {
        "title": "Related Neutrals",
        "colors": [
          "Joanna®",
          "Rolling Fog - Light™"
        ]
      },
      {
        "title": "Coordinating Colours",
        "colors": [
          "Nether Red™",
          "Arras™"
        ]
      },
      {
        "title": "Contrasting Accent",
        "colors": [
          "Heat™"
        ]
      },
      {
        "title": "Related Darks",
        "colors": [
          "Purple Brown™"
        ]
      }
    ],
    "color": "#E0C8B3",
    "hsl": {
      "h": 28,
      "s": 42.1,
      "l": 79
    },
    "categories": [
      "Re:mix Flat Emulsion Paint"
    ]
  }
];

export function toID(text) {
  if (text === undefined) {
    return undefined;
  }
  return text.replaceAll(/[:™®'’]/g, "").normalize("NFD").replace(/\p{Diacritic}/gu, "").replaceAll(/[- &]+/g, " ").replaceAll(" ", "-").toLowerCase().replaceAll("color", "colour").replaceAll("gray", "grey");
}

export function toKey(text) {
  if (text === undefined) {
    return undefined;
  }
  const id = toID(text);
  const initialVowel = ["a", "e", "i", "o", "u"].includes(id[0]) ? "e" : "";
  return initialVowel + id.replaceAll(/[-.aeiou]/g, "");
}

/**
 * A complete match on the ID or number or alias of a color
 * @param { string | number } color 
 */
export function toColor(color) {
  if (typeof color === "number") {
    color = `${color}`;
  }
  if (typeof color == "string") {
    // The ids collection now includes explicit ids, and IDs generated from title, aliases, and number.
    const id = toID(color);
    const byID = colors.find(c => c.ids.includes(id));
    if (byID !== undefined) {
      return byID;
    }

    return undefined;
  }

  return color;
}

function getKeys(color, toK) {
  return [...(new Set([toK(color.id), color.number, toK(color.title), ...(color.aliases ?? []).map(a => toK(a))].filter(x => x !== undefined)))];
}

// Create IDs from titles if an ID not already present
for (const color of colors) {
  if (color.id === undefined) {
    color.id = toID(color.title);
  }
  color.ids = getKeys(color, toID); //[...(new Set([toID(color.id), toID(color.number), toID(color.title), ...(color.aliases ?? []).map(a => toID(a))].filter(x => x !== undefined)))];
  color.keys = getKeys(color, toKey); //[...(new Set([toKey(color.id), toKey(color.number), toKey(color.title), ...(color.aliases ?? []).map(a => toKey(a))].filter(x => x !== undefined)))];
  color.soundKeys = getKeys(color, toSoundKey);
  
  color.words = new Set();
  for (const id of color.ids) {
    for (const word of id.split("-")) {
      if (["a", "s", "of"].includes(word)) { continue; }
      color.words.add(word);
    }
  }
}

export const wordPrefixes = new Trie();
colors.map(clr => [...clr.words].map(word => wordPrefixes.setValues(word, [clr])));
export const keyPrefixes = new Trie();
colors.map(clr => [...clr.words].map(word => keyPrefixes.setValues(toKey(word), [clr])));
export const soundPrefixes = new Trie();
colors.map(clr => [...clr.words].map(word => soundPrefixes.setValues(toSoundKey(word), [clr])));

// Sort colors by length of ID and then alphabetically
/**
 * @param { Color } a 
 * @param { Color } b 
 * @returns number
 */
export function colorCompare(a, b) {
  if (a.id.length !== b.id.length) {
    return a.id.length - b.id.length;
  }
  return a.id.localeCompare(b.id);
};

colors.sort(colorCompare);

////////////////////////////////////////////////////////////////////////////////////

/** @typedef { { selected?: boolean; id: string; title: string; color: string; } } Color */
/** @typedef { Color[] } ColorChoice */
/** @typedef { ColorChoice[] } Palette */

////////////////////////////////////////////////////////////////////////////////////

export function isNeutral(color) {
  return color.categories.includes("Neutral");
}

export function isWhite(color) {
  return color.categories.includes("White");
}

export function isColorful(color) {
  return !isNeutral(color) && !isWhite(color);
}

export function isDark(color) {
  // return color.lightness !== undefined && color.lightness < 20;
  return color.hsl.l < 30;
}

function connections(color, title) {
  if (color.connections == undefined) { return []; }

  return color.connections.filter(c => c.title == title).flatMap(c => c.colors.map(a => toColor(a)));
}

function connecting(color, connection, connection2 = connection, filter = (_clr) => true) {
  color = toColor(color);

  const result = new Set();

  for (const clr of connections(color, connection)) {
    result.add(clr);
  }

  for (const clr of colors) {
    if (filter(clr) && connections(clr, connection2).includes(color)) {
      result.add(clr);
    }
  }

  return { start: color, title: connection, colors: [...result] };
}

export function coordinating(color) {
  return connecting(color, "Coordinating Colours", "Coordinating Colours", (clr) => isColorful(clr));
}

export function contrasting(color) {
  return connecting(color, "Contrasting Accent");
}

export function neutrals(color) {
  return connecting(color, "Related Neutrals", "Coordinating Colours", (clr) => isNeutral(clr) && !isWhite(clr));
}

export function whites(color) {
  return connecting(color, "Related Whites", "Coordinating Colours", (clr) => isWhite(clr));
}

export function darks(color) {
  return connecting(color, "Related Darks", "Coordinating Colours", (clr) => isDark(clr));
}

export function* colorsByPrefix(prefix) {
  const prefixID = toID(prefix);
  for (const color of colors) {
    for (const id of color.ids) {
      if (id.startsWith(prefixID)) {
        yield color;
      }
    }
  }
}

export function* colorsByPrefixKey(prefix) {
  const prefixID = toKey(prefix);
  for (const color of colors) {
    for (const id of color.keys) {
      if (id.startsWith(prefixID)) {
        yield color;
      }
    }
  }
}

export function* colorsBySuffix(suffix) {
  const suffixID = toID(suffix);
  for (const color of colors) {
    for (const id of color.ids) {
      if (id.endsWith(suffixID)) {
        yield color;
      }
    }
  }
}

export function* colorsByIncludes(text) {
  let id = toKey(text);
  // keys only have a vowel at the beginning, remove it for an includes search
  if (id[0] === "e") {
    id = id.substring(1);
  }
  for (const color of colors) {
    for (const key of color.keys) {
      if (key.includes(id)) {
        yield color;
      }
    }
  }
}

export function* colorsBySound(text) {
  let id = toSoundKey(text);
  for (const color of colors) {
    for (const key of color.soundKeys) {
      if (key.includes(id)) {
        yield color;
      }
    }
  }
}

export function toColorDefault(name, fallback) {

  if (name === undefined || name.length === 0) {
    return toColor(fallback ?? "Carys");
  }

  let color = toColor(name);
  if (color !== undefined) {
    return color;
  }

  // for (const clr of colorsByPrefix(name)) {
  //   return clr;
  // }

  // for (const clr of colorsBySuffix(name)) {
  //   return clr;
  // }

  for (const [clr] of getPrefixMatches(name)) {
    return clr;
  }

  for (const clr of colorsByPrefixKey(name)) {
    return clr;
  }

  for (const clr of colorsBySound(name)) {
    console.log("SOUND", name);
    return clr;
  }
  
  for (const clr of colorsByIncludes(name)) {
    return clr;
  }

  // Use shorter string if we don't find anything
  return toColorDefault(name.substring(0, name.length - 1), fallback);
}

/**
 * @param { Palette } palette 
 * @returns { string }
 */
export function paletteToQuery(palette) {
  return palette.map(colors => colors.map(color => color.id + (color.selected ? "*" : "")).join("|")).join(",");
}

/**
 * @param { Palette } palette 
 * @returns { string }
 */
export function paletteToText(palette) {
  return palette.map(colors => colors.map(color => color.title.replaceAll(/[™®]/g, "") + (color.number !== undefined ? ` (${color.number})` : "") + (color.selected ? "*" : "")).join(" | ")).join(",\n");
}

/**
 * 
 * @param { string } line 
 * @param { (string) => Color } textToColor 
 * @returns { ColorChoice }
 */
function lineToColorChoice(line, textToColor = toColorDefault) {
  let seenSelected = false;
  return line.replaceAll("/", "|").split("|").map(name => name.trim()).filter(name => name.length > 0).map(name => {
    const o = parseTitle(name);
    const isMarkedSelected = o.selected;
    name = o.title;

    let clr = textToColor(name);
    if (!seenSelected && isMarkedSelected) {
      seenSelected = true;
      clr = Object.create(clr);
      clr.selected = true;
    }
    return clr;
  });
}

/**
 * @param { string } text 
 * @param { (string) => Color } textToColor 
 * @returns { Palette }
 */
export function textToPalette(text, textToColor = toColorDefault) {
  return text.trim().replaceAll(/([,\r\n]\s*)+/g, "\n").split("\n").map(line => line.trim()).filter(line => line.length > 0).map(line => lineToColorChoice(line, textToColor));
}

/**
 * Takes all the unique colors from the palette and creates a new palette
 * where every row is the set of unique colors
 * and the selected color is different on each row
 * @param { Palette } palette 
 */
export function matrix(palette) {
  const colors = [...new Set(palette.flatMap(line => line.map(color => { delete color.selected; return color; })))];
  const result = [];
  for (let index = 0; index < colors.length; ++index) {
    const line = [...colors];
    line[index] = Object.create(line[index]);
    line[index].selected = true;
    result.push(line);
  }
  return result;
}

export const colorExpression = /^\s*(?<title>[^*()]*[^*() ])\s*(?<selected1>[*])?\s*([(](?<number>\d+)[)])?\s*(?<selected>[*])?\s*$/;

/**
 * 
 * @param { string } text 
 * @returns 
 */
export function parseTitle(text) {
  const selected = text.includes("*");
  text = text.replaceAll("*", "");
  
  const end = text.indexOf("(");
  const title = (end >= 0) ?
    text.substring(0, end).trim() :
    text.trim();

  return { title, selected };
}

/**
 * 
 * @param { string } text 
 * @returns 
 */
export function toSoundKey(text) {
  if (text === undefined) {
    return undefined;
  }
  text = text.trim().toLowerCase().replaceAll(/[:™®'’]/g, "").normalize("NFD").replace(/\p{Diacritic}/gu, "").replaceAll(/[- &]+/g, " ");

  const sound_G = "k";

  const replacements = [
    [/((boro(ug)?)|(b[eu]rg))h?\b/g, "bro"],
    [/rq(u?)/g, "rk"],
    [/qui(s?)\b/g, "ky"],
    [/\bmc/g, "mak"],
    [/\bmac[ei]l/g, "makil"],
    [/\bhalf/g, "harf"],

    [/d?ge\b/g, "tch"],
    [/dg([eiy])/g, "tch$1"],
    //[/j\b/g, "tch"],
    [/j/g, "tch"],
    [/\btsar/g, "zar"],
    [/\bczar/g, "zar"],
    ["slavic", "slavik"], // because of following rule
    [/(.)[vw]i[ct][sz]?\b/g, "$1vitch"],
    [/([aeiouyn])t?s?ch\b/g, "$1tch"],
    ["ć", "tch"],
    ["cz", "tch"],
    [/cek\b/g, "tchek"],

    ["tch", "sh"],

    ["š", "sh"],
    ["sz", "sh"],
    [/ch([aeiy])/g, "sh$1"],

    [/([aeiouy])gg([aeiouy])/g, `$1${sound_G}$2`], // too early for this, but can't use lookbehind in the next rule (for old browser support), so here it is
    [/g[ae]i/g, "gay"],
    [/giv/g, `${sound_G}iv`], // too soon
    [/get/g, `${sound_G}et`], // too soon
    [/g([eiy])/g, "sh$1"], // g followed by softening vowel, but not double g

    [/[st](i[oa])/g, "sh$1"],

    [/e[ae]\b/g, "y"],
    [/a[ei]\b/g, "y"],
    [/c([eiy])/g, "s$1"],
    [/se\b/g, "s"], // already converted c to s
    [/([^lrwyaeiou\r\n\t ])le\b/g, "$1el"],
    [/re\b/g, "er"],

    [/([aeiouy])([^ey\r\n\t ]+)es\b/g, "$1$2s"], // anes -> ens

    [/\bx([aiou])/g, "sh$1"], // chinese X
    [/\bx([ey])/g, "z$1"], // greek X

    [/\bwho/g, "ho"],
    [/\bwh/g, "w"],

    ///////////////////// VOWEL COMPRESSION ////////////////////////////////////
    [/e\b/g, ""],        // remove e from end of word (in English it typically changes preceding vowel, not pronounced independently)
    [/[aeiou]+/g, "e"],  // aeiou are vowels
    [/y([^e\r\n\t ])/g, "e$1"], // y followed by consonant is a vowel
    [/y\b/g, "e"],       // word-ending y is a vowel
    ///////////////////////////////////////////////////////////////////////////
    
    [/([^e\r\n\t ])wh/g, "$1w"], // w binds to following h when preceded by a consonant
    [/w([^e\r\n\t h]|\b)/g, "$1"],  // w followed by consonant or end of word is unnecessary

    [/([aeiouy])([^td\r\n\t ])ed\b/g, "$1$2d"], // bawled -> bald, bothered -> botherd (but patted/padded unchanged)
    
    [/mb\b/g, "m"],
    [/gne([^e\r\n\t ]+)\b/g, "ne$1"],
    [/gn\b/g, "n"],
    [/\b[kgp]n/g, "n"],
    
    ["x", "ks"],
    ["ch", "k"],
    [/egh([^e\r\n\t ]|\b)/g, "e$1"], // high, light
    ["wre", "re"],
    ["th", "f"],
    ["ph", "f"],
    
    [/[bp]+/g, "b"],
    [/[fv]+/g, "v"],
    [/[cgkq]+/g, "k"],
    [/[dt]+/g, "d"],
    [/[mn]+/g, "n"],
    [/[sz]+/g, "s"],

    ["j", "y"],
    
    [/er([^ey\r\n\t ]|\b)/g, "e$1"], // remove insignificant R

    [/eh\b/g, "e"], // remove unnecessary h
    ["kh", "k"], // remove unnecessary h
    
    [/(.)\1+/g, "$1"], // condense runs

    [/\b([bcdfghjklmnpqrstvwxyz])\b/g, "$1e"], // single letter consonant gets a vowel
  ];

  for (const replacement of replacements) {
    text = text.replaceAll(replacement[0], replacement[1]);
    // console.log(replacement, text);
  }
  return text;
}

/**
 * 
 * @param { Map } map 
 * @param { object } item 
 * @param { Iterable<string> } keys
 */
export function addToMultimap(map, item, keys) {
  for (const key of keys) {
    let list = map.get(key);
    if (list === undefined) {
      list = [item];
      map.set(key, list);
    } else {
      if (!list.includes(item)) {
        list.push(item);
      }
    }
  }
  return map;
}

export const words = new Map();
colors.map(color => addToMultimap(words, color, color.words));

export const sounds = new Map();
colors.map(color => addToMultimap(words, color, [...color.words].map(w => toSoundKey(w))));

/**
 * 
 * @param { string } text 
 */
export function toSounds(text) {
  return toSoundKey(text).split(" ");
}

function hasWordPrefix(color, input) {
  for (const word of color.words) {
    if (word.startsWith(input)) {
      return true;
    }
  }
  return false;
}

function commonPrefix(t1, t2) {
  const result = [];
  for (let index = 0; index < Math.min(t1.length, t2.length); ++index) {
    if (t1[index] === t2[index]) {
      result.push(t1[index]);
    } else {
      break;
    }
  }
  return result.join("");
}

export function getPrefixMatches(text) {
  const inputs = toID(text).split("-").map(word => ({ word, key: toKey(word), sound: toSoundKey(word) }));
  const result = {};

  for (const input of inputs) {
    checkPrefixes(input, wordPrefixes, "word", 0.5);
    // checkPrefixes(input, keyPrefixes, "key", 1.0);
//    if (text.length > 3) {

      // checkPrefixes(input, soundPrefixes, "sound", 3.0);
//    }
  }

  return Object.entries(result).map(([id, score]) => [toColor(id), score]).sort(([color, score], [color2, score2]) => {
    if (score > score2) return -1;
    if (score < score2) return +1;
    return colorCompare(color, color2);
  });

  function checkPrefixes(input, collection, prop, scale) {
    const i = input[prop];
    const values = collection.getValuesWithError(i);
    for (const o of values) {
      const score = result[o.value.id] ?? 0;
      const increment = o.key.length;// + (o.isTerminal ? (o.key.length === i.length ? o.key.length : 0.1) : 0);
      result[o.value.id] = score + scale * increment;
    }
  }
}

export function getPrefixMatches2(text) {
  const inputs = toID(text).split("-").map(word => ({ word, sound: toSoundKey(word) }));
  const result = {};
  
  checkMap(words, "word", 1.2);
  if (text.length > 2) {
    checkMap(sounds, "sound", 1.0);
  }

  return Object.entries(result).map(([id, count]) => [toColor(id), count]).sort(([color, count], [color2, count2]) => {
    if (count > count2) return -1;
    if (count < count2) return +1;
    return colorCompare(color, color2);
  });

  function checkMap(map, prop, scale) {
    const entries = [...map.entries()];

    for (const [key, list] of entries) {
      for (const input of inputs) {
        const value = input[prop];
        const prefix = commonPrefix(value, key);
        if (prefix.length > 0) {
          const increment = scale * (prefix.length / value.length) * (prefix.length / key.length);
          for (const color of list) {
            let count = result[color.id] ?? 0;
            count += increment;
            result[color.id] = count;
          }
        }
      }
    }
  }
}


/** @typedef { { key: string; prefix?: Set; inner?: Set; } & Record<string, LetterNode> } LetterNode */

/**
 * Adds each character of the `word` from the start to the end as a node in the tree
 * @param { LetterNode } root 
 * @param { string } word 
 * @param { object[] } items 
 * @param { "prefix" | "inner" } collectionName
 */
export function setWord(root, word, items, collectionName = "prefix") {
  let current = root;
  for (const key of word) {
    /** @type LetterNode */
    let next = current[key];
    if (next === undefined) {
      next = { key };
      next[collectionName] = new Set(items);
      current[key] = next;
    } else {
      /** @type Set */
      const collection = next[collectionName] ?? (next[collectionName] = new Set());

      for (const item of items) {  
        collection.add(item)
      }
    }
    current = next;
  }
}

/**
 * Adds each character of the `word` as a node in the tree from the start of the string under the `prefix` collection
 * and adds each substring of the word from position n to the end of the string under the `inner` collection
 * @param { LetterNode } root 
 * @param { string } word 
 * @param { object[] } items 
 */
export function setEveryWord(root, word, items) {
  for (let index = 0; index < word.length; ++index) {
    setWord(root, word.substring(index), items, index === 0 ? "prefix" : "inner");
  }
}

/**
 * @param { LetterNode } root 
 * @param { string } word 
 * @param { object[] } items 
 * @returns { LetterNode[] }
 */
export function getWord(root, word) {
  /** @type LetterNode[] */
  const result = [];
  let current = root;
  for (const key of word) {
    /** @type LetterNode */
    let next = current[key];
    if (next === undefined) {
      return result;
    }
    result.push(next);
    current = next;
  }
  return result;
}


// colors.map(clr => [clr.title, toSoundKey(clr.title.replaceAll(/[:™®'’]/g, "").normalize("NFD").replace(/\p{Diacritic}/gu, "").replaceAll(/[- &]+/g, " "))]);

// toSoundKey("Ceviche");
// toSoundKey("Arras");